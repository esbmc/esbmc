...
**Extracted internals cache.** ESBMC materialises its bundled clang headers, operational models and helper scripts on disk before clang can read them. These are cached in `/tmp/esbmc-cache-<uid>/` (mode 0700, one entry per payload keyed by a content digest), so a given build extracts each payload once rather than once per run — the suite no longer fills `/tmp` with thousands of `esbmc-headers-*` directories. Entries total a few MB and are safe to delete at any time; they are re-extracted on demand, and `systemd-tmpfiles` ages out stale ones from old builds on its own. Set `ESBMC_CACHE_DIR` to relocate the cache (useful for CI), or configure `-DESBMC_CACHE_IN_HOME=On` to use `$XDG_CACHE_HOME/esbmc` (default `~/.cache/esbmc`) instead, which survives reboots but grows until cleared by hand. On a shared machine the per-uid root is created 0700 and its ownership is confirmed before use, so a leftover or wrong-mode directory of that name is declined and ESBMC falls back to per-run extraction rather than reusing it.
