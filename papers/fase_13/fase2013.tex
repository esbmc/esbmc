...
We can thus model the constraints and properties of the $for\_each$ function  of the algorithm library as:
%
\begin{equation}
...
languages (e.g., Java). In this case, we have to replicate the methods and atributes of the base
classes to the inherited class to have direct access them, which thus disallows the direct use 
of tools already created to other programming languages
...
In constrat to other approaches (e.g.,~\cite{Blanc07}), ESBMC++ is able to 
...
with a ``conversio error''. ESBMC++ also handles  virtual destructors successfully 
...
between ESBMC++ and LLBMC (Low-Level Bounded Model Checker) 
using a set of standard C++ benchmarks.
