...
and \textit{general utilities}. The language support category is compose by four 
libraries that deal with exception handling (i.e., exception library), 
types information (i.e., typeinfo library), dynamic allocation of memory 
(i.e., new library), and numeric limits (i.e., limits library). 
The diagnostic category is composed by the stdexcept library, which implements 
standard exceptions (i.e., logic and runtime errors). Figure~\ref{figure:stdexcept}
shows the hierarchical structure of the stdexcept library.
...
To define the types of errors, we assume that $e$ represents an element within the set 
$E$ such that for each $e$ exists one class $\varphi$ that represents the erro given that all 
$\varphi$ contains a constructor that sets a message to be thrown if there is any errors during 
the program execution. To identify these errors during the verification, whenever one of the 
constructors of these classes is called, an $assert(0)$ is throwing, which forces ESBMC++ to 
identify this error and report it in the counter-example. We can thus represent the model 
of the stdexcept library as:
