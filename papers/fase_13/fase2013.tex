...
three variables: a variable $P$ called pointer, which contains the memory address 
to the corresponding element $T$ in the container, a variable $N$ called position, 
which contains the index value pointed by the iterator in the container, and a 
variable $P$ called source, which contains the memory address to the first 
...
where $P$ is a memory address where the elements are stored in the container, 
$C.size$ is the total number of elements in the container, and $C.capacity$ 
...
makes use of an iterator $It$ (position) that points to the desired 
insertion position; a template value $T$ (value) with the element 
to be inserted and an integer $N$ (quantity) that informs the amount 
of elements to be inserted in the desired position.

\[\begin{array}{ll}
%\label{eqnarray:transformations}
cont.insert(It, T, N) \Longrightarrow & cont'.size = cont.size \\
  & \wedge \: *(position + 0) = value \\
  & \ldots \\
  & \wedge \: *(position + N) = value \\
\end{array}\]
...
Let $It\{first\}$ be an iterator that marks the first element 
to be inserted, and let $It\{end\}$ be another iterator that 
points to the first element after the end of the sequence to be inserted 
in the required position. Therefore, we have:\\

\[\begin{array}{ll}
%\label{eqnarray:transformations}
cont.insert(It, T, N) \Longrightarrow & cont'.size = cont.size + (last - first)\\
  & \wedge \: *position = *first \\
\end{array}\]

The same model above is valid for pointers $P$ (first) and $P$ (last). 
...
\[\begin{array}{ll}
\label{erase1-model}
\mathit{cont.erase} (\mathit{It}) \Longrightarrow & \mathit{cont'.size} = \mathit{cont.size} - \mathit{1}\\
  & \wedge \: \mathit{position'} = \mathit{position} + \mathit{1} \\
\end{array}\]
...
\[\begin{array}{ll}
\label{erase2-model}
\mathit{cont.erase} (\mathit{It}, \mathit{first}, \mathit{last}) \Longrightarrow & 	\mathit{cont'.size} = \mathit{cont.size} -(\mathit{last} - \mathit{first})\\
  & \wedge \:	\mathit{position'} = \mathit{last} \\
\end{array}\]
...
$N\{*It\}$, $N\{*P\}$ or $N\{\mathit{C_n}\}$.	The structure of iterators 
is treated differently from other types. The model is the following:
$\mathit{It} = \{ \mathit{pointer}, \mathit{pos}, \mathit{cont\_pos}\}$,
where $pointer$ is a memory address that points to the 
real position of the required element in the container 
(pointed by the iterator), $pos$ is the iterator indexed 
internally in the container, and $cont\_pos$ is a memory 
