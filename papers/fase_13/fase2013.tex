...
makes use of an iterator $It\{\iota\}$ that points to the desired 
insertion position; a template value $T\{\tau\}$ with the element 
to be inserted and an integer $N\{\theta\}$ that informs the amount 
of elements to be inserted in the desired position.

\[\begin{array}{ll}
%\label{eqnarray:transformations}
cont.insert(\iota, \tau, \theta) \Longrightarrow & cont'.size = cont.size \\
  & \wedge \: *\iota = \tau \\
  & \ldots \\
  & \wedge \: *(\iota + \theta - 1) = \tau \\
\end{array}\]
...
Let $It\{\alpha_0\}$ be an iterator that marks the first element 
to be inserted, $It\{\alpha_k\}$ be another iterator that 
points to the first element after the end of the sequence to be inserted 
in the required position and let $N\{k\}$ be the length of the array $[\alpha_0\, \alpha_k)$. 

Therefore, we have:\\

\[\begin{array}{ll}
%\label{eqnarray:transformations}
cont.insert(\iota, \alpha_0, \alpha_k) \Longrightarrow & cont'.size = cont.size + k\\
  & \wedge \: *\iota = *\alpha_0 \\
  & \ldots \\
  & \wedge \: *(\iota + k - 1) = *(\alpha_k - 1)
\end{array}\]

The same model above is valid for pointers $P\{\alpha_0\}$ and $P\{\alpha_k\}$. 
...
\[\begin{array}{ll}
\label{erase1-model}
cont.erase (\iota) \Longrightarrow & cont'.size = cont.size - 1\\
  & \wedge \: \iota' = \iota + 1 \\
\end{array}\]
...
\[\begin{array}{ll}
\label{erase2-model}
cont.erase (\iota, \alpha_0, \alpha_k) \Longrightarrow & 	cont'.size = cont.size - k\\
  & \wedge \:	\iota' = \alpha_k \\
\end{array}\]
...
internally in the container, and $cont\_n$ is a memory 
