...
\blurb{I think this should be removed -- it just says that there's an operator symbol for each macro. Or don't I get it?}
...
\end{equation}
\blurb{remove up to here}

\blurb{spell out the actual calls: signal($\sigma$, f); rais($\sigma$)?}
...
derived class, is chosen in runt-time using the late binding.
