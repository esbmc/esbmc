...
previous approaches are not able to deal with~\cite{Blanc07,Florian12,PrabhuMBIG11}.
...
Modeling C++ features like inheritance and polymorphism make static analysis difficult to
implement.
Inheritance is one of the main benefits of object-oriented which allows a class (derived class)
absorbs data and behavior
from existing classes (base class) and enhances it with new features.
This is the vital concept behind the creation of reusable software and has received
much attention because is one of the promissing ways to improve software quality.

C++ and Java allow single inheritance, when derived classes have only one base class.
In addition, C++ allows multiple inheritance, when a class may inherit from
one or more base classes, probably not related. This is a difficult in relation to other
object-oriented languages, such as Java, disallowing the direct use of tools already created for other
languages.

When a class inherits from a base class features not virtual, this is called replicated inheritance. If exists a path from class X to class Y
whose first edge is virtual, we call this a shared inheritance.
A formal description to represent the relationship between the classes can be described by the Class Hierarchy Graph (CHG). 
This graph $\zeta$ is formed of a tuple $ \textless \textbf{C}, \prec_{s}, \prec_{r}>$, \textbf{C} being the set of classes. $
\prec_s $, referring to shared inheritance edges and $\prec_r$, are replicated inheritance edge. 
$\prec_{s}$ and $\prec_{r}$ is the set of edges representing inheritance relationship between classes and $\subseteq \textbf{C} \times \textbf{C}$.
$\prec_{sr} = \prec_s \bigcup \prec_r$ and $\leq_{sr} = (\prec_{sr})^*$. ($\textbf{C}, \leq_{sr}$) a partially ordered set and $\leq_{sr}$ antisymmetric.

Our tool creates an intermediate representation. For instance, Figure~\ref{figure:multiple-inheritance-IR} shows an multiple inheritance with two sub-objects: 
($\textbf{C}, <Derived, Base1>)$ 
and ($\textbf{C}, <Derived, Base2>)$.
...
This model transforms all the classes in structures and joins all methods and attributes of its parent classes. This approach has the advantage in verification due the direct access,
allowing an easier validation, as the tool must not look for attributes or methods from base classes on each access. 
On the other hand, we replicate information to any new class, wasting memory resources.



Polymorphism permits variable instances to be bound to references of different types according to the
structure of the inheritance hierarchy ~\cite{Alexander02}. It is the principle whereby two or more derived classes from the same base class can invoke
methods with same signature but distinct behaviors, specialized for each derived class, using for this one reference to each object of this base class type.
The decision of which method must be used, according to the type of the derived class, is choosen in execution timing using the late binding (falta citação).
The advantage is creation of reusable code by programmers by changing only specific features from the base class.
%explicar como a ferramenta trata polimorfismo, utilizar diagrama UML.
%Inicialização rapida de construtor
%Herança virtual
%Explicar herança virtual com diagramas
%Explicar destrutor virtual
%Nao suporta dynamic_cast
Our tool does not support dynamic cast.


% Polymorphism is a mechanism whereby we can abstract over type. It is the principle whereby two or more derived classes from the same base class can invoke
% methods with same signature but distinct behaviors. For instance, we can write polymorphism in Napier~\cite{Morrison87,MorrNap87} as
%
% \begin{equation}
% \label{polymorphism-eq}
% let\;id = proc [t:type] (x : t \rightarrow t);\;x
% \end{equation}
%
% Id procedure is parametrised by the type into brackets from t to t. Thus this procedure is in fact an infinite number of identity procedures which implements several types.
% The advantage of the polymorphic abstraction reside in the context of software re-use. Using this feature we can write one procedure for all types, instead of different one
% for each type.
% Thereby this feature improves the complexity for static verification.
...
%-----------------------------------
The experimental results is divided in two parts. The setup is described on section 4.1, followed by a comparison between ESBMC++ and LLBMC using a benchmark provided by the authors.

\subsection{Experimental Setup}

The benchmark used on the tests consist in 1113 test cases. Around 290 tests come from the Deitel Book~\cite{Deitel} and the others were developed to test all the features the C++ provides. The test suite is divided in 9 modules:

\begin{itemize}
 \item Algorithm: Tests involving algorithm lib.
 \item Cpp: General tests.
 \item Deque: Tests involving deque lib.
 \item Inheritance: Tests involving inheritance and polymorphism.
 \item List: Tests involving list lib.
 \item Stream: Tests involving I/O stream libs.
 \item String: Tests involving string lib.
 \item Try\_catch: Tests involving exception Handling
 \item Vector: Tests involving vector lib.
\end{itemize}

All the experiments were conducted on an otherwise idle Intel Core i7-2600, 3.40 GHz with 24 GB of RAM running Ubuntu 64-bits. For all the modules, the individual time limit and memory limit for each test has been set to 900 seconds (15 minutes) and 24 GB (22 GB of RAM and 2 GB of virtual memory). The times given were measured using the unix time command.

\subsection{Comparison to LLBMC}

This subsection describes the evaluation of ESBMC++ against another BMC tool developed by Merz et al.~\cite{Florian12}. The ~\ref{table:results-of-the-comparison-between-ESBMC-and-LLBMC} summarize the results.
...
\newblock {\em In Proceedings of the 13th International Symposium on Software Reliability Engineering (ISSRE '02)} pp. 207-. 2002,
IEEE Computer Society, Washington, DC, USA.

% \bibitem{Morrison87}
% R.~Morrison, A. L.~Brown, R.~Carrick and R. C. H.~Connor
% \newblock {Polymorphism, persistence and software re-use in a strongly typed object-oriented environment.}
% \newblock {\em Softw. Eng. J. 2, 6. } pp.\ 199-204, November, 1987.
%
% \bibitem{MorrNap87}
% R.~Morrison
% \newblock {Napier language reference manual}.
% \newblock {\em Universit of St. Andrews } Scotland, 1987.
