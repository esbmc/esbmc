...
inside the try block, with a few adaptations:

\begin{enumerate}
  \item Before the try block, a CATCH instruction with an
        empty map (which will be filled later on the typechecking).
  \item The code inside the try block.
  \item A CATCH instruction to represent the end of the try block
        and beginning of the catch block.
  \item a GOTO instruction pointing to the code after the catch block.
\end{enumerate}

The GOTO after the catch block will change if an exception is thrown,
it will point to the corresponding catch, otherwise it will remain the same.
After typechecking the try block, ESBMC++ typechecks the catch block that
will contain one or more catches. Again, the AST will be created based on
the code inside the catches with only one adaptation: a GOTO instruction is
inserted by the end of each catch code pointing to the code after the catches.
Each catch code will be assigned a label so that ESBMC++ can decide which catch
should be called during the symbolic execution phase if an exception is thrown.
By the end of the catch block, the map of the first CATCH instruction is
inserted before the try block code is filled with the label created
for each catch mapped on the type of the exception. Figure~\ref{figure:try-catch-goto}
...
for later usage. The idea behind the use of a stack is
that we may have try-catch blocks inside other
try-catches block and ESBMC++ should always handle
the most internal first. Following the symbolic execution
...
starting with the variables $\mathit{T}$ and pointers $\mathit{P}$.
The syntax for $\mathit{T}$ values is the following:
%
\[\begin{array}{r@{\:\:}c@{\:\:}l}\label{element-semantics}
\\[-5ex]
\mathit{T} & ::= & \: \mathit{T_t} \: | \: \mathit{T_C} \: | \: const \: \mathit{T} \: |
\: volatile \: \mathit{T} \: |  \: restrict \: \mathit{T} \: |\: \mathit{*P} \: \\
\end{array}
\]
%
Where $\mathit{T_t}$ is a variable of type $\mathit{T}$ at a THROW instruction,
$\mathit{T_C}$ is a variable of type $\mathit{T}$ at a catch, $const$,
$volatile$ or $restrict$ are qualifiers to variables of type $\mathit{T}$ and
$\mathit{*P}$ is the value stored in the $\mathit{P}$ position of the memory.
For $\mathit{P}$ (memory address values), the syntax is as follows:
%
\[\begin{array}{r@{\:\:}c@{\:\:}l}\label{pointer-semantics}
\\[-5ex]
\mathit{P}  & ::= & \: \mathit{p} \: | \: \mathit{P_{[]}} \: \\
\end{array}
\]
%
where $\mathit{p}$ is a pointer variable and  $\mathit{P_{[]}}$ is a
memory address that stores the beginning of an array.
...
a \textit{bad\_alloc} exception, (b) the operator
\textit{dynamic$\_$cast} can throw a \textit{bad\_catch}
exception, and (c) the \textit{typeid} function can throw a
\textit{bad\_typeid} exception.
Those exceptions are built-in on C++ and are supposed to be
...
To model the behavior of exception handling in ESBMC++, we define
several functions the first is $match$. The function $match$ returns
the handle that matches the thrown type and makes use of the set of thrown
types and the set of handles types.

The function $match$ is composed by several functions called $rule$,
each function $rule$ returns either true if the rule is valid, or nil
if the rule is false and makes use of the throw type and the set of handles.
Inside the $match$ function, when one of the rules is true, the following
rules are ignored.

Finally, each $rule$ can call a function called $choose$. The function $choose$
is responsible to update the GOTO instruction after the try block to point to the
corresponding catch. The function $choose$ makes use of $\mathit{T_C}$.

The functions $rule_1$ and $rule_2$ define a match if $T_{t}$ is equal to $T_{C}$ (ignoring
const-volatile-restrict qualifiers).

\[\begin{array}{ll}
rule_1(T_{t1},T_{c1}, T_{c2}, \ldots, T_{cN}) \Leftrightarrow
  (T_{t} = T_{C}) \rightarrow choose(T_{C}) \\
\end{array}\]

\[\begin{array}{ll}
rule_2(T_{t1},T_{c1}, T_{c2}, \ldots, T_{cN}) \Leftrightarrow
  (T_{t} = cvr \: T \: \wedge \: T_{C} = T)\rightarrow choose(T_{C})
\end{array}\]
...
\[\begin{array}{ll}
rule_3(T_{t1},T_{c1}, T_{c2}, \ldots, T_{cN}) \Leftrightarrow
  (T_{t} = T_{f()} \: \wedge \: T_{C} = T_{f()})\rightarrow choose(T_{C}) \\
\end{array}\]

\[\begin{array}{ll}
rule_4(T_{t1},T_{c1}, T_{c2}, \ldots, T_{cN}) \Leftrightarrow
  (T_{t} = T_{[]} \: \wedge \: T_{C} = T*)\rightarrow choose(T_{C}) \\
\end{array}\]
...
\[\begin{array}{ll}
rule_5(T_{t1},T_{c1}, T_{c2}, \ldots, T_{cN}) \Leftrightarrow
  (T_{t} = T_{1} \: \wedge \: T_{C} = T_{2} \: \wedge \: base(T_{2}, T_{1}))\rightarrow choose(T_{C}) \\
\end{array}\]
...
\[\begin{array}{ll}
rule_6(T_{t1},T_{c1}, T_{c2}, \ldots, T_{cN}) \Leftrightarrow
  (T_{t} = T_{1}* \: \wedge \: T_{C} = T_{2}* \: \wedge \: implicit\_conv(T_{1}, T_{2}))\rightarrow choose(T_{C}) \\
\end{array}\]
...
\[\begin{array}{ll}
rule_7(T_{t1},T_{c1}, T_{c2}, \ldots, T_{cN}) \Leftrightarrow
  (T_{t} = T* \: \wedge \: T_{C} = VOID*)\rightarrow choose(T_{C})
\end{array}\]
...
\[\begin{array}{ll}
rule_8(T_{t1},T_{c1}, T_{c2}, \ldots, T_{cN}) \Leftrightarrow
  (T_{t} = T \: \wedge \: T_{C} = T_{ellipsis})\rightarrow choose(T_{C})
\end{array}\]
...
\[\begin{array}{ll}
rule_9(T_{t1},T_{c1}, T_{c2}, \ldots, T_{cN}) \Leftrightarrow
