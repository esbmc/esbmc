...
One other important feature from object-oriented program that our tool
...
The advantage is creation of reusable code by programmers by changing only 
specific features from the base class.
%explicar como a ferramenta trata polimorfismo, utilizar diagrama UML.
The intermediate representation grant an model to treat polymorphism in 
order it simplifies the class hierarchy,
easing the access to methods with same name without ambiguity between 
base and derived classes.
For instance, Figure~\ref{figure:uml_diagram} shows an UML diagram 
representing
the Shape class hierarchy. The Rectangle class relation can be 
formalized by
$\left(C, \left\langle Rectangle, Shape \right\rangle \right)$. Our tool can
handle with the polymorphic area method using this representation, 
see Figure~\ref{figure:IR_uml_rec}.

Our tool supports the indirect inheritance, a class inherits features 
from a derived class throw one or more classes
...
thus Square class can access features from Shape class, 
but they are not directly connected. We treat this difficult in verification
looking for the features using depth search from derived to base classes and adding to our
...
In object-oriented programming the use of \textit{shared inheritance} is common. 
...
this need for implementation belongs to derived classes.
Otherwise, if a class is only virtual it must have an implementation or
the verication fails. ESBMC also handles with virtual destructors successfully. Our tool does 
not support dynamic cast.
