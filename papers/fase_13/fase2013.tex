...
with replicated and shared inheritance.
...
One other important feature from oriented-object program that our tool
supports is the polymorphism. Polymorphism permits variable instances to be
...
derived class, is choosen in runt-time using the late binding (falta citação).
...
The intermediate representation grant an model to treat polymorphism in order it simplifies the class hierarchy,
easing the access to methods with same name without ambiguity between base and derived classes.
For instance, Figure~\ref{figure:uml_diagram} shows an UML diagram representing
the Shape class hierarchy. The Rectangle class relation can be formalized by
$\left(C, \left\langle Rectangle, Shape \right\rangle \right)$. Our tool can
handle with the area method polymorphism this representation, see Figure~\ref{figure:IR_uml_rec} .

Our tool supports the indirect inheritance, a class inherits features from a derived class throw one or more classes
not directly connected. In Figure~\ref{figure:uml_diagram}, 
$\left(C, \left\langle Square, Rectangle, Shape \right\rangle \right)$
Square class can access features from Shape class, but it is not directly connected. This is one
difficult to verification that we treat by looking for the tree using depth search from derived to base classes and adding to our
IR, see Figure~\ref{figure:IR_uml_square}.


\begin{figure}[ht]
\centering
\includegraphics[scale=0.28]{figures/heranca_simples}
\caption{UML diagram from Shape class hierarchy}
...
%Inicializa\E7\E3o rapida de construtor
%Heran\E7a virtual
%Explicar heran\E7a virtual com diagramas
