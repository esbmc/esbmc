...
check whether $\sigma$ is a valid signal. To raise a function associated to signal
$\rho$, we simply select it from the signal array $a$ and then check whether $\rho$
is a valid signal and the function associated to $\rho$ is different than \verb|NULL|.
The $raise\left(\sigma\right)$ function can thus be modeled in our operational 
model as:
%
\begin{equation}
...
\]
%
Where $\mathit{t}$ is an elment of type $\mathit{T}$ and 
...
\]
%
Where $\mathit{begin}$ and $\mathit{end}$ are methods
...
%
\begin{figure*}[ht] \centering
\includegraphics[scale=0.25]{figures/stl-iterator} 
...
languages (e.g., Java). In this case, we have to replicate the methods and attributes 
...
The intermediate representation of our tool provides a model to treat polymorphism in
