...
This graph $\zeta$ is formed of a tuple $<\textbf{C},  \prec_{s}, \prec_{r}>$, \textbf{C} being the set of classes. $
\prec_s \subseteq \textbf{C} \times \textbf{C}$, referring to shared inheritance edges and $\prec_r \subseteq 
\textbf{C} \times \textbf{C}$, are replicated inheritance edge. $\prec_{s}$ and $\prec_{r}$ is the set of edges representing inheritance relationship between classes.
$\prec_{sr} = \prec_s \bigcup \prec_r$ and $\leq_{sr} = (\prec_{sr})^*$. ($\textbf{C}, \leq_{sr}$), being $\leq_{sr}$ a partially ordered set.
A class X is said to be base class of another Y iff exists a nonempty path from X to Y in the CHG.
...
allowing an easier validation. On the other hand, we replicate information to any new class, wasting memory resources.
