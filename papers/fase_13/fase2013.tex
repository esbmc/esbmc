...
The $find$ function looks for a certain element within a range while 
$for\_each$ function applies a given function to a range. For this, 
we assume that $\eta$ is a $NULL$ element and $I$ represents a range, 
such that $\left\{first, last, n, i\right\}\:\subset\:I$. $first$ 
represents the pointer to the beginning of the range, $last$ represents 
...
within the range. For the formalization of the $for\_each$ function, 
we assume that $f$ be a pointer to function and for the $find$ function 
we assume that $\alpha$ be a pointer to the value sought
%, and $\beta$ is a Boolean value that assumes $true$ if the element sought is contained in the range and $false$ otherwise.
 We can thus model the $for\_each$ of the 
algorithm library as:
%
\begin{equation}
...
to accumulate the values that belong to a numerical sequence. To given an exmample 
...
\caption{Diagram about hierarchical structure of the stdexcept library.}
...
error message to be thrown if there is any errors during the program execution. 
To identify these errors during the verification, whenever one of the 
constructors of these classes is called, an $assert(0)$ is throwing, which forces ESBMC++ to 
identify this error and report it in the counter-example. The operational model of the 
stdexcept library can thus represented as:
...
the $string$ library. In the model constructed to represent this library, we implemented it structure through the class called string that contains 
...
string object as array of char, so that all heuristics that must be observed in handling vectors are therefore approached in this model. 
...
Therefore, firstly we assume $S$ as the set of string variables, $\eta$ as $NULL$ element, $c$ as character non-deterministic, the set $\left\{n, i, pos, length\right\} \subset N$, 
and we define that s representing a string, the same has two attributes: the string size described by $length$, a character sequence that 
represent it described by $str$. Keep this in mind, can be defined a constructor method that creates a string containing $n$ times the 
character $c$ and, furthermore the find method that search a character within a certain string from a given position described by $pos$. 
Therefore, we can model the $constructor$ of the string library as:

\begin{equation}
\label{cconstructor-string}
C := \left [ \begin{array}{ll} 
                \bigvee^{n}_{i=0} store\left(str, i, c_{0}\right) 
              \end{array} \right ]  \\ 
\end{equation}
%
\begin{equation}
...
\label{pfind-string}
P := \left [ \begin{array}{ll} 
                \bigwedge^{length}_{i=0} ite\left(select\left(str, i\right) = c_{0}, i, -1\right)
              \end{array} \right ]  \\ 
\end{equation}
...
\begin{minipage}[b]{.48\linewidth}
  \centering
  \centerline{\includegraphics[scale=0.5]{figures/multiple_inheritance_graph.png}}
%  \vspace{1.5cm}
...
\begin{minipage}[b]{.48\linewidth}
  \centering
  \centerline{\includegraphics[scale=0.6]{figures/indirect_inheritance.png}}
%  \vspace{1.5cm}
  \centerline{(b)}\medskip
\end{minipage}
%
...
Jing Yang et al. present a translation tool called Class Hierarchy Representation Object Model Extension (CHROME) that is targeted towards 
making static program analyzers for C++ easier to write and provide more precise results. This model makes a source-to-source transformation 
from a C++ program with inheritance into a semantically equivalent without inheritance by treating the inheritance as separate memory regions 
that are linked to each other via additional base class and derived class pointer fields. 
This transformation comprises a clarifier which makes implicit C++ features explicit and the CHROME model. 
This approach was verified with a bug finding tool called F-SOFT~\cite{Fsoft}. 
CHROME has a different memory behavior from the original program and does not permit the use of low-level primitives like memset. 
The CHROME-lowered C program is three to five times bigger than the size of the original program ~\cite{Yang12}.
...
\bibitem{CVC07}
C.~Barrett and C.~Tinelli, ``{CVC}3,'' in \emph{Intl. Conf. on Computer Aided Verification (CAV}, LNCS 4590, pp. 298--302, 2007.
...
\bibitem{Boolector09}
R.~Brummayer and A.~Biere, ``Boolector: An efficient {SMT} solver for
  bit-vectors and arrays,'' in \emph{Intl. Conf. on Tools and Algorithms for the Construction and Analysis of Systems (TACAS)}, LNCS 5505, pp. 174--177, 2009.

\bibitem{Z08}
L.~M. de~Moura and N.~Bj{\o}rner, ``Z3: An efficient {SMT} solver,'' in
  \emph{Intl. Conf. on Tools and Algorithms for the Construction and Analysis of Systems (TACAS)}, LNCS 4963, pp. 337--340, 2008.
...
\newblock {\em International Conference on Compiler Construction (CC).} March 2012.
