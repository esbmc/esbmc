...
This subsection describes the evaluation of ESBMC++ against another BMC tool developed by Merz et al.~\cite{Florian12}. The ~\ref{table:results-of-the-comparison-between-ESBMC-and-LLBMC} summarize the results. Here, \textit{N} is the number os tests,  \textit{L} is the total lines of code on each suite, \textit{Time} is the total time of verification on each module, \textit{P} the number of positive results on the verification, \textit{FP} the number of false positive results on the verification, \textit{FN} the number of false negative results on the verification, \textit{Crash} the number of crashes during the verification of each module, \textit{TO} the number of Time-outs (>900 seconds) and \textit{MO} the number of Memory-outs (>24 GB).
