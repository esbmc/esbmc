...
programming languages (e.g., Java) since it disallows the direct use of tools 
already created to other programming languages. To deal with inheritance in ESBMC++, 
we simply replicate the methods and attributes of the base classes to the inherited 
class to have direct access to them.
...
we call this as \textit{shared inheritance}. A formal description to represent 
...
and $\prec_r$ are \textit{replicated inheritance} edge. The symbols $\prec_{s}$ and $\prec_{r}$
are the set of edges that represent the inheritance relationship between
classes and $\subseteq \mathit{C} \times \mathit{C}$.
$\prec_{sr} = \prec_s \bigcup \prec_r$ and $\leq_{sr} = (\prec_{sr})^*$. ($\mathit{C}, \leq_{sr}$)
are a partially ordered set~\cite{Neggers99} and $\leq_{sr}$ is anti-symmetric (i.e., if one 
element A of the set preceeds B, the opposite relation could not exist).
...
replicated and shared inheritance as shown in Figure~\ref{figure:multiple-inheritance-IR}.
...
This IR model transforms all the classes in structures and joins all
...
from the same base class can invoke methods with the same signature but distinct behaviors, 
...
The intermediate representation provides a model to handle polymorphism in
...
as shown in (\ref{area-class-c}) and (\ref{area-class-p})3), using the background
theories of the SMT solvers.
%
\begin{equation}
\label{area-class-c}
C := \left [ \begin{array}{ll}
                w_{0} = 10 \wedge h_{0} = 10 \\
		\wedge j_{1} = \left(j_{0} = Rectangle@Rectangle\right) \\
		\wedge j_{2}.width = \left(j_{1}.width = 10\right) \\
		\wedge j_{3}.height = \left(j_{2}.height = 10\right) \\
		\wedge j_{4} = \left(j_{3} = Rectangle@Square\right) \\
		\wedge j_{5} = \left(j_{4} = Square@Square\right) \\
		\wedge j_{6}.width = \left(j_{5}.width = 10\right) \\
		\wedge j_{7} = \left(j_{6}.width * j_{6}.width\right) \\
              \end{array} \right ],  \\
\end{equation}
%
\begin{equation}
...
Figure~\ref{figure:IR_uml_square} shows this feature in symex representation.
...
\begin{lstlisting}[style=nonumbers]
this@1!0&0#1 == (struct Rectangle *)(&dynamic_1_value)
w@1!0&0#1 == 10
h@1!0&0#1 == 10
dynamic_1_value&0#1 == 
   (dynamic_1_value&0#0 WITH 
   [Rectangle@vtable_pointer:=&virtual_table::Rectangle@Rectangle])
dynamic_1_value&0#2 == 
   (dynamic_1_value&0#1 WITH 
   [width:=10])
dynamic_1_value&0#3 == 
   (dynamic_1_value&0#2 WITH 
   [heigth:=10])
dynamic_1_value&0#4 == 
   (dynamic_1_value&0#3 WITH 
   [Rectangle@vtable_pointer:=&virtual_table::Rectangle@Square])
dynamic_1_value&0#5 == 
   (dynamic_1_value&0#4 WITH 
   [Square@vtable_pointer:=&virtual_table::Square@Square])
dynamic_1_value&0#6 == 
   (dynamic_1_value&0#5 WITH 
   [width:=10])
sqre@1!0&0#1 == &dynamic_1_value
this@1!0&0#1 == &dynamic_1_value
return_value@1!0&0#1 == 
   dynamic_1_value&0#6.width * dynamic_1_value&0#6.width
(assert)!__ESBMC_alloc&0#1[POINTER_OBJECT(&dynamic_1_value)] => 
   return_value@1!0&0#1 == 100\end{lstlisting}
\end{minipage}
...
and supports the default constructor creation. ESBMC++ currently
supports dynamic cast between primitive types, same classes and from a derived 
class to a base. The tool also handles with cast to a reference type, verifying the 
correct use of bad\_cast thrown by dynamic cast.
