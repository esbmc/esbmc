...
\caption{ESBMC/ESBMC++ Architecture (ESBMC-specific components shown dashed).}
...
\[
\begin{array}{l}
  i=j      \Rightarrow \textit{select}(\textit{store}(a,i,v),j)=v \\
  i \neq j \Rightarrow \textit{select}(\textit{store}(a,i,v),j)=\textit{select}(a,j)
\end{array}
\]
...
is the source code parser; ESBMC++ takes C++ source code as input and creates
most of the intermediate representation of the program, which will be the base
for the remaining steps of the program verification. The parser is
...
In the next step, the IRep tree is converted into goto expressions; this conversion
simplifies the representation of the C++ program (e.g, replacement of \emph{switch} and \emph{while}
by \emph{if} and \emph{goto} statements), and handles the unrolling of loops and the
...
Additionally, in this step most of the exception handling is carried out, such as the insertion of GOTO-instructions for the original
\textit{throw} statements and exception specification for function calls.
...
C++ relies on a collection of powerful standard libraries to provide
much of the functionality programmers require. In principle, we could use
the (available) sources during the verification, but their optimized
...
Apart from the STL handling, the verification of C++ programs with
...
instantiated to avoid creating a duplicate representation of a previously
...
\\  & | &
     \mathit{C.erase(It)} \:|\: \mathit{C.erase(It, It)} \:|\: \mathit{C.search(It)}
\\[0.5ex]
   P  & ::= &
     p \:|\: P (+ \:|\: - ) P
       \:|\: \mathit{C.array}
...
$\mathit{*It}$ to denote the value stored in the underlying container
at the position pointed to by the iterator $\mathit{It}$; 
this is an abbreviation for $\mathit{(It.source.array)[It.pos]}$. 
$\mathit{*P}$ is the value stored in the $\mathit{P}$ position of the memory.

%%% Iterators can be added and subtracted.  
$\mathit{C.begin}$ and $\mathit{C.end}$
are methods that return iterators which point to the beginning and the ending
of a container, respectively.  Most container operations also return an
iterator pointing to the new focus element after the operation rather than
simply returning an updated container. For example, for vectors $\mathit{C.erase}$
...
from each other, some of their methods will vary too,
...
variable of type $P$ called \emph{source}, which 
points to the underlying container. Figure~\ref{figure:stl-iterator} gives
an overview of our operational model for the STL sequantial containers.
...
As part of the single static assignment (SSA) transformation, 
side-effects on the iterators and containers
are made explicit, so that operations return 
new iterators and containers as result. 
As an example, consider a container $c$ with the
method call \textit{c.insert} that returns an iterator result and
makes use of an iterator $i$ that points to the desired
(insertion) position; a template value $t$ with the element
to be inserted and an integer $n$ that denotes the number of times
the element is to be inserted.
The statement
$\mathit{c.insert}(i,t,n);$ (which discards the returned iterator)
thus becomes $(c',i')=\mathit{c.insert}(i,t,n);$ (where the side effects
are explicit).

\begin{figure}[ht] \centering
\includegraphics[scale=0.3]{figures/stl-iterator}
\caption{Operational model of the STL sequential containers.}
\label{figure:stl-iterator}
\end{figure}

\vspace*{-1ex}

The translation function $\cal C$ describes the constraints relating 
the ``before'' and ``after'' versions of the respective model variables.
...
%
\[\begin{array}{lll}
\multicolumn{3}{l}{{\cal C}((c',i')=c\mathit{.insert}(i, t, n)):=}\\
  & \multicolumn{2}{l}{\wedge\:c'\mathit{.size} = c\mathit{.size}+n}\\
  & \wedge\: c'\textit{.array}=&\!\!\!\!\textit{store}(\ldots(\textit{store}(\\
  & & \textit{store}(\ldots(\textit{store}(c.\textit{array}, i\textit{.pos}, t),\\
  & & \:\:\:\:\:\:\ldots,\\
  & & \:\:\:\:\:\:i\textit{.pos}+n-1, t), \\
  & & i\textit{.pos}+n, \textit{select}(c\textit{.array}, i\textit{.pos})), \\
  & & \ldots, \\
  & & c\textit{.size}+n-1, \textit{select}(c\textit{.array}, c\textit{.size}-1))\\
  & \multicolumn{2}{l}{\wedge\:i'\textit{.source}=c'}\\
  & \multicolumn{2}{l}{\wedge\:i'\textit{.pos}=i\textit{.pos}+n}\\
\end{array}\]

The main effect of the \textit{insert} method is thus captured by the second
equality that describes the contents of the container array $c'\mathit{.array}$
after the insertion in terms of update operations to the container array
$c\mathit{.array}$ before the insertion.

There is another version of the insert method. Here
it is possible to insert a sequence of elements in the desired
insertion position, using both iterator (or even pointer) bounds to
select the sequence from another container.
Let $i_0$ be an iterator that marks the first element
to be inserted, $i_k$ be another iterator that
...
%
\[\begin{array}{lll}
\multicolumn{3}{l}{{\cal C}((c',i')=c\mathit{.insert}(i, i_0, i_k)):=}\\
  & \multicolumn{2}{l}{\wedge\:k = i_k\mathit{.pos}-i_0\mathit{.pos}+1}\\
  & \multicolumn{2}{l}{\wedge\:a = i_0\mathit{.source.array}}\\
  & \multicolumn{2}{l}{\wedge\:c'\mathit{.size} = c\mathit{.size}+k}\\
  & \wedge\: c'\textit{.array}=&\!\!\!\!\textit{store}(\ldots(\textit{store}(\\
  & & \textit{store}(\ldots(\textit{store}(c.\textit{array},\\
  & & \:\:\:\:\:\:i\textit{.pos}, \textit{select}(a, i_0\textit{.pos})),\\
  & & \:\:\:\:\:\:\ldots,\\
  & & \:\:\:\:\:\:i\textit{.pos}+k-1, \textit{select}(a, i_k\textit{.pos}-1)),\\
  & & i\textit{.pos}+k, \textit{select}(c\textit{.array}, i\textit{.pos})), \\
  & & \ldots, \\
  & & c\textit{.size}+k-1, \textit{select}(c\textit{.array}, c\textit{.size}-1))\\
  & \multicolumn{2}{l}{\wedge\:i'\textit{.source}=c'}\\
  & \multicolumn{2}{l}{\wedge\:i'\textit{.pos}=i\textit{.pos}+k}\\
\end{array}\]

%%%% I don't think this is really complete -- sometimes iterators become
%%%% invalidated after an update to the container...
Note that this also implicitly induces the two properties that $[i_0\,
i_k)$ is non-empty (i.e., that $k>0$ holds) and that $i_0$ and $i_k$ are iterators
over the same underlying container (i.e., that
$i_0\mathit{.source.array}=i_k\mathit{.source.array}$ holds), 
although this container can be
different from the one we are inserting into.
...
The following model shows the \textit{erase} method that deletes a single element:
%
\comment{
\[\begin{array}{ll}
%\label{erase1-model}
...
\multicolumn{3}{l}{{\cal C}((c',i')=c\mathit{.erase}(i)):=}\\
  & \multicolumn{2}{l}{\wedge\:c'\mathit{.size} = c\mathit{.size}-1}\\
  & \wedge\: c'\textit{.array}=&\!\!\!\!\textit{store}(\ldots(\textit{store}(c.\textit{array},\\
  & & i\textit{.pos}, \textit{select}(c\textit{.array}, i\textit{.pos}+1)), \\
  & & \ldots, \\
  & & c\textit{.size}-2, \textit{select}(c\textit{.array}, c\textit{.size}-1))\\
  & \multicolumn{2}{l}{\wedge\:i'\textit{.source}=c'}\\
  & \multicolumn{2}{l}{\wedge\:i'\textit{.pos}=i\textit{.pos}}\\
\end{array}\]

Note that this implicitly induces the property that $i$ is an iterator
over $c$ (i.e., that $i\mathit{.source}=c$ holds).
...
\textit{insert} method; the details are omitted here.
\comment{
\[\begin{array}{ll}
%\label{erase2-model}
...
value (the element stored itself). 
\comment{
It can be considered as values
...
C++ features like inheritance and polymorphism
...
(CHG). This graph is represented by a triple
...
$\langle C, \emptyset,
 \{(\mathit{Rectangle, Shape)}, (\mathit{Rectangle, Display})\}
\rangle$.
...
or more classes not directly connected. 
In Figure~\ref{figure:uml_diagram}, we have
$\mathit{Square}\leq_{\mathit{sr}}\mathit{Rectangle}$
and 
$\mathit{Rectangle}\leq_{\mathit{sr}}\mathit{Shape}$.
...
1 & algorithm   & 130  & 3376  & 996   & 63   & 38  & 16  & 13   & 0    & 0    & 0     & 22964  & 53  & 45  & 1   & 5   & 0    & 24  & 2\\ % OK
\hline
2 & deque       & 43   & 1239  & 238   & 19   & 20  & 0   & 4    & 0    & 0    & 0     & 8585   & 16  & 17  & 0   & 0   & 1    & 9   & 0\\ % OK
\hline
3 & vector      & 146  & 6853  & 2714  & 95   & 37  & 3   & 11   & 0    & 0    & 0     & 7234   & 91  & 38  & 1   & 3   & 4    & 6   & 3\\ % OK
\hline
4 & list        & 70   & 2292  & 3928  & 25   & 25  & 3   & 17   & 0    & 0    & 0     & 2562   & 5   & 26  & 5   & 30  & 0    & 0   & 4\\ % OK
\hline
5 & queue       & 14   & 328   & 177   & 7    & 7   & 0   & 0    & 0    & 0    & 0     & 45     & 6   & 7   & 0   & 1   & 0    & 0   & 0\\ % OK
\hline
6 & stack       & 12   & 286   & 82    & 6    & 6   & 0   & 0    & 0    & 0    & 0     & 45     & 6   & 6   & 0   & 0   & 0    & 0   & 0\\ % OK
\hline
7 & inheritance & 51   & 3460  & 311   & 28   & 17  & 1   & 2    & 3    & 0    & 0     & 122    & 32  & 12  & 1   & 3   & 3    & 0   & 0\\ % OK
\hline
8 & try\_catch  & 67   & 4743  & 45    & 17   & 41  & 7   & 2    & 0    & 0    & 0     & 4      & 0   & 1   & 0   & 0   & 66   & 0   & 0 \\ % OK
\hline
9 & stream      & 66   & 1831  & 1892  & 51   & 13  & 0   & 2    & 0    & 0    & 0     & 11     & 17  & 13  & 0   & 35  & 1    & 0   & 0\\ % OK
\hline
10 & string     & 233  & 4921  & 46186 & 100  & 112 & 5   & 16   & 0    & 0    & 0     & 37     & 6   & 121 & 4   & 102 & 0    & 0   & 0\\ % OK
\hline
11 & cpp        & 343  & 26624 & 1817  & 269  & 38  & 7   & 25   & 4    & 0    & 0     & 3260   & 235 & 24  & 10  & 56  & 15   & 2   & 1\\ % OK
\hline\hline
  &             & 1175 & 55953 & 58386 & 680  & 354 & 42  & 92   & 7    & 0    & 0     & 44869  & 467 & 310 & 22  & 235 & 90   & 41  & 10\\ % OK
\hline
\end{tabular} }
\end{center}
...
\footnote[1]{esbmc -\/-unwind \textit{B}  -\/-no-unwinding-assertions -I /libraries/ --timeout 15m}
...
in the verification process; this is a situation that also happens in other suites.
...
ESBMC++ verified all suites in 58386 seconds (approximately 16 hours)
and gave the right results for 1034 out of 1175 programs (88\%) while LLBMC verified all suites
...
\newblock In {\em FMCAD}, pp.\ 121--128, 2010.
...
\bibitem{Visser05}
W.~Visser and P.~Mehlitz 
\newblock Model Checking Programs with Java PathFinder. 
\newblock In {\em SPIN}, LNCS 3639, pp. 27, 2005.
