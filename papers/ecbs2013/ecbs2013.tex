...
\multicolumn{3}{l}{{\cal C}((c',i')=c\mathit{.insert}(i, t, n)):=}\\
  & \multicolumn{2}{l}{\:c'\mathit{.size} = c\mathit{.size}+n}\\
...
in the required position, $\mathit{acon}$ be another container whom iterators $\mathit{it}_0$ 
and $\mathit{it}_k$ are pointing to and let $k$ be the length of the array
$[\mathit{it}_0\, \mathit{it}_k)$.
Thus, we have:

\[\begin{array}{lll}
\multicolumn{3}{l}{{\cal C}((c',i')=c\mathit{.insert}(it_{0}, it_{k})):=}\\
  & \multicolumn{2}{l}{\:c'\mathit{.size} = c\mathit{.size}+k}\\
  & \wedge\: c'\textit{.array}=&\!\!\!\!\textit{store}(\ldots(\textit{store}(\\
  & & \textit{store}(\ldots(\textit{store}(c.\textit{array}, i\textit{.pos}, \\
  & & \:\:\:\:\:\:\:\:\:\:\:\:select(acon\textit{.array}, it_{0}\textit{.pos})),\\
  & & \:\:\:\:\:\:\ldots,\\
  & & \:\:\:\:\:\:i\textit{.pos}+k-1, select(acon\textit{.array}, it_{k}\textit{.pos})), \\
...
The same model above is valid for pointers $pt_0$ and $pt_k$.
...
\multicolumn{3}{l}{{\cal C}((c',i')=c\mathit{.erase}(i)):=}\\
  &  c'\textit{.array}=&\!\!\!\!\textit{store}(  \\
  & & \textit{store}(c.\textit{array}, i\textit{.pos}, select(c.array, i.pos+1)) \\
  & & \ldots, \\
  & & c\textit{.size}-2, \textit{select}(c\textit{.array}, c\textit{.size}-1))\\
  & & \wedge\:c'\mathit{.size} = c\mathit{.size}-1\\
  & \multicolumn{2}{l}{\wedge\:i'\textit{.source}=c'}\\
  & \multicolumn{2}{l}{\wedge\:i'\textit{.pos}=i\textit{.pos}+n}\\
\end{array}\]

It is also possible to delete a number $k$ of elements from the container by
marking the bounds with iterators ($it_0$ and $it_k$, considering both pointing to the container $con$).
 It works similarly to the equivalent
\textit{insert} method:


\[\begin{array}{lll}
\multicolumn{3}{l}{ {\cal C}((c',i')=c\mathit{.erase}(it_0, it_k)):= }\\
  & \: c'\textit{.array}=&\!\!\!\!\textit{store}(\ldots(\textit{store}) \\
  & & \textit{store}(\ldots(\textit{store}(c.\textit{array}, it_0\textit{.pos}, \\
  & & \:\:\:\:\:\:\:\:\:\:select(c.array, it_0.pos+k)),\\
  & & \:\:\:\:\:\:\ldots,\\
  & & \:\:\:\:\:\:it_0\textit{.pos}+k-1, \\
  & & \:\:\:\:\:\:\:\:\:\:select(c.array, it_0.pos+2*k-1)), \\
  & & it_0\textit{.pos}+k, \textit{select}(c\textit{.array}, it_0\textit{.pos+2*k})), \\
  & & \ldots, \\
  & & c\textit{.size}-k-1, \textit{select}(c\textit{.array}, c\textit{.size}-1))\\
\end{array}\]
