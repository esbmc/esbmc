...
% affiliations
\author{\IEEEauthorblockN{Mikhail Ramalho$^1$, Mauro Freitas$^1$, Felipe Rodrigues$^1$, \\
	Hendrio Marques$^1$, Lucas Cordeiro$^1$, and Bernd Fischer$^2$}
\IEEEauthorblockA{$^1$ Electronic and Information Research Center, Federal University of Amazonas, Brazil \\ $^2$ Electronics and Computer Science, University of Southampton, UK}
...
LLBMC~\cite{Florian12}, a bounded model checker for C/C++ programs that is based
on SMT solvers.
...
We developed a simplified representation of the C++
...
return the actual and maximal size, respectively of the containter $C$.
...
three variables: a variable of type $P$ called \emph{ptr}, which contains the memory address
to the corresponding element $T$ in the container, a variable $N$ called \emph{pos},
which contains the index value pointed by the iterator in the container, and a
variable of type $P$ called \emph{src}, which contains the memory address to the first
...
On C++, templates are special functions/methods and classes that are
defined using generic types. These generic types allow the developer
to create functions/methods or classes that can adapt to one or more
different types without the need to rewrite the function/method or
class to each type. Figure~\ref{figure:function-template-example} and
Figure~\ref{figure:class-template-example} show simple examples of
template usage. In those examples, we can see that a template can assume
different types (integer, float and char).
...
\end{lstlisting}
\end{minipage}
\caption{Function template example: returns the bigger value for two generic types.}
\label{figure:function-template-example}
\end{figure}
...
\end{lstlisting}
\end{minipage}
\caption{Class Template example: a vector with one element.}
\label{figure:class-template-example}
\end{figure}

On ESBMC++, the verification of C++ codes with templates is
divided in two steps: template creation and template instantiation,
both during typechecking phase. On template creation, ESBMC++ finds
the template declaration and creates an internal representation of
the class or function/method and flag the types as generic (this is
important for the next step). No other representation is created at
this step as ESBMC++ doesn't know yet, which types we be instantiated
on the templates.

On the template instantiation step, ESBMC++ found a template usage
with a specified type. Now, ESBMC++ will create a new internal
representation of the function/method or class with the instantiated
type. This new representation is not a template anymore. At this point,
ESBMC++ starts to keep track of the generic template definition and
the instantiated functions/methods or classes: when a new template is
instantiated, ESBMC++ looks if it had been previously instantiated before,
if it hasn't happened, the generic template definition is used to create
a new representation to the new instantiated template.

%------------------------------------------------------
\subsection{Template Specialization}
%------------------------------------------------------

On C++ codes, templates can be created to generic types usage and can be
specialized to specific types. The Figure~\ref{figure:function-template-specialization-example}
shows an example of function template specialization (it can also be done
for classes though).
...
template< >
T example (int a, int b) {
  return b;
}

int main()
{
  // it will return the second value: 2
  int i = example(1,2);

  // it will return the first value: 1.0
  float f = example(1.0, 2.0);

  return 0;
}
\end{lstlisting}
\end{minipage}
\caption{Function template specialization example: if the type is integer, returns the second value instead of the first value.}
\label{figure:function-template-specialization-example}
\end{figure}

For ESBMC++, templates specialization are handled on the template creation
phase. When ESBMC++ finds a template specialization, it first search for a
generic template (as it is required to specialize a template), if there is
such generic template, then it creates a new internal representation of the
template specialization as if it was instantiated. There is no change on the
