...
# Pylint suppressions for this property-test module:
# - protected-access / unnecessary-dunder-call: the tests deliberately exercise
#   the model's internals — they read its "protected" representation fields
#   (_sign, _int, _exp, _is_special, _digit_count, _from_int) and call its
#   dunder methods (__eq__, __lt__, ...) directly, which is the whole point of
#   validating the model against CPython.
# - no-value-for-parameter: when Codacy/Prospector scans the repository as a
#   whole, pylint resolves `import decimal` to ESBMC's operational model
#   (src/python-frontend/models/decimal.py, whose Decimal takes four positional
#   arguments) instead of the standard library, so every genuine
#   cpython_decimal.Decimal(...) call looks like it is missing arguments. The
#   name clash is unavoidable (the model file must be named decimal.py).
# pylint: disable=protected-access,unnecessary-dunder-call,no-value-for-parameter
...
def _digit_count(n):
    """Digit count of a non-negative int, via the model's instance method."""
    return ModelDecimal(0, n, 0, 0)._digit_count()
