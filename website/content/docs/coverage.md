...
Coverage analysis uses verification internally to determine reachability, but it
reports in reachability terms rather than verification ones. Each goal is
announced as `✓ REACHED` or `- UNREACHED`, the witness trace is headed
`[Reachability trace]` rather than `[Counterexample]`, and the run closes with a
coverage verdict instead of `VERIFICATION SUCCESSFUL` / `VERIFICATION FAILED`:

```bash
$ esbmc example.c --multi-property --assertion-coverage
...
✓ REACHED: 'assertion 1 at file example.c line 3 column 3 function main'
✓ REACHED: 'assertion 2 at file example.c line 4 column 3 function main'
Coverage goals: 2 ✓ 2 reached
...
COVERAGE ANALYSIS COMPLETE
```

The closing line says whether the percentages above were actually measured:

| Verdict | Meaning |
|---|---|
| `COVERAGE ANALYSIS COMPLETE` | Every coverage goal got a definitive reachable/unreachable answer, so the percentages are exact. |
| `COVERAGE ANALYSIS INCOMPLETE` | Some goals were never decided, or the program was not fully explored, so the percentages are **lower bounds**. |

`INCOMPLETE` prints the reason underneath — for example `--multi-fail-fast` cut
the run short, the solver returned no result, `--smt-formula-only` solved
nothing, the assertion-instance total was non-deterministic, or the run hit its
maximum k step. Both outcomes exit 0: an incomplete measurement is not a program
defect.

A coverage run does not verify the program, so a claim outside the coverage
instrumentation that ESBMC happens to refute along the way is not reported as a
failure. It is not swallowed either — the run warns and names each such claim,
and re-running without the coverage flag reports them properly.

**What matters:** the coverage percentage, qualified by the `COMPLETE` /
`INCOMPLETE` line.
...
![Figure: Example LCOV-style HTML coverage report generated by cov-report.py.](/images/cov.png)
