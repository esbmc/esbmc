...
It creates the build folder, optionally installs the Boolector and/or Bitwuzla SMT solvers, and makes esbmc available globally. The script also supports building the Python frontend.
...
- **LLVM version**: Install LLVM 21 via Homebrew: `brew install llvm@21`. Do not use `DOWNLOAD_DEPENDENCIES=ON` as it downloads Linux versions incompatible with macOS.
