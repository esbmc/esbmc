...
Both modes generate, into an `esbmc-ctest/` subdirectory of the current directory:
...
Use `--ctest-output-dir <dir>` to write them somewhere else; the directory is created if it does not exist. ESBMC refuses to overwrite any file it did not generate itself (identified by the `Auto-generated by ESBMC` marker on the first line) and aborts generation with an error rather than destroying your work.

> **Behaviour change.** These files were previously written into the current directory, which silently destroyed an existing `CMakeLists.txt` ([issue #6214](https://github.com/esbmc/esbmc/issues/6214)). Automation that expected them in the working directory needs either `--ctest-output-dir .` or a path update.
...
Generate coverage report using `gcovr`. The root is `../..` (the directory ESBMC was run from) because the generated project references the original sources by absolute path from `esbmc-ctest/`:
