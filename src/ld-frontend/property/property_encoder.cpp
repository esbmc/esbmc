...
// Minimal boolean expression parser supporting:
...
code_blockt property_encoder::encode_mutual_exclusion(const LdProperty &p)
{
  ...
  code_assertt asrt = make_assert(not_exprt(conjunction), p);
...
{
  code_blockt blk;
  code_assertt asrt = make_assert(parse_bool_expr(p.expression, p), p);
  blk.copy_to_operands(asrt);
  return blk;
}
...
code_blockt property_encoder::encode_response(const LdProperty &p)
{
  ...
  // assert ctr <= max_scans
  code_assertt asrt = make_assert(binary_relation_exprt(ctr, "<=", limit), p);
  blk.copy_to_operands(asrt);
  return blk;
}
