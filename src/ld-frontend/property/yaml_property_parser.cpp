...
{
  if (s == "mutual_exclusion") return PropertyKind::mutual_exclusion;
  if (s == "invariant")        return PropertyKind::invariant;
  if (s == "response")         return PropertyKind::response;
  if (s == "absence")          return PropertyKind::absence;
  if (s == "reachability")     return PropertyKind::reachability;
...
std::vector<LdProperty> YamlPropertyParser::parse(const std::string &path)
{
  ...
  for (const auto &node : root["properties"])
  {
    ...
    if (!node["kind"])
      throw LdPropertyParseError(path + ": property '" + p.id + "' missing 'kind'");
    ...
    if ((p.kind == PropertyKind::response || p.kind == PropertyKind::reachability) &&
    ...
    case PropertyKind::mutual_exclusion:
      if (!node["variables"])
        throw LdPropertyParseError(path + ": mutual_exclusion property '" + p.id + "' missing 'variables'");
    ...
    case PropertyKind::reachability:
      if (!node["expression"])
        throw LdPropertyParseError(path + ": property '" + p.id + "' missing 'expression'");
      p.expression = node["expression"].as<std::string>();
      break;

    case PropertyKind::response:
      if (!node["trigger"])
        throw LdPropertyParseError(path + ": response property '" + p.id + "' missing 'trigger'");
      if (!node["response"])
        throw LdPropertyParseError(path + ": response property '" + p.id + "' missing 'response'");
      if (!node["max_scans"])
        throw LdPropertyParseError(path + ": response property '" + p.id + "' missing 'max_scans'");
