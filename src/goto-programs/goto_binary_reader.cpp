  ...
  goto_functionst &dest)
{
  ...
  char hdr[4] = {0, 0, 0, 0};
  // Bounded read: never copies more than sizeof(hdr) bytes into the fixed-size
  // buffer; the gcount() check below confirms a full header was read before
  // any byte is inspected (CWE-120/CWE-20). Flawfinder: ignore
  ...
  if (
    got >= static_cast<std::streamsize>(sizeof(hdr)) &&
    is_cbmc_goto_magic(uhdr))
