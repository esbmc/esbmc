...
/// is `size * sizeof(alloctype)` for every kind we check. This matters for
/// typed requests: the frontend lowers a bare `malloc(sizeof(T))` to `size ==
/// 1, alloctype == T` (never a raw byte count), so without the scaling a large
/// `T` would be checked as `1 <= K` and slip through. A byte-count request such
/// as `malloc(n)` or `n * sizeof(T)` keeps `alloctype == char` (sizeof 1), so
/// the scaling is the identity there.
