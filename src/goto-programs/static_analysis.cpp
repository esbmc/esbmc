...
static_analysis_baset::get_next(working_sett &working_set)
{
      ...
      it++)
  {
    ...
    if(l->is_function_call())
    {
      ...
      do_function_call_rec(
        l, code.function, code.operands, new_values, goto_functions);
...
void static_analysis_baset::do_function_call(
  ...
  const std::vector<expr2tc> &arguments [[maybe_unused]], // XXX -- why?
  statet &new_state)
{
  ...
  const goto_functionst &goto_functions)
{
  ...
  else if(is_if2t(function))
  {
    ...
    do_function_call_rec(
      l_call, ifval.true_value, arguments, new_state, goto_functions);

    do_function_call_rec(
      l_call, ifval.false_value, arguments, *n2, goto_functions);
  ...
  else if(is_dereference2t(function))
  {
        ...
        it++)
    {
      if(is_object_descriptor2t(*it))
      {
        ...
        std::unique_ptr<statet> n2(make_temporary_state(new_state));
        do_function_call_rec(
          l_call, obj.object, arguments, *n2, goto_functions);
        merge(new_state, *n2);
      }
    }
  }
...
void static_analysis_baset::fixedpoint(const goto_functionst &goto_functions)
{
  // do each function at least once
