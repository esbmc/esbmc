...
// A Clang short-circuit temporary T selects its value at the assume point via a
// conditional branch between the value assignment and a constant fallback:
//     T = nondet
//     IF <guard> GOTO L_short
//     T = <value>
//     GOTO L_end
//   L_short: T = <const>        // false/0 for &&, true/1 for ||
//   L_end:
// i.e. T == (<guard> ? <const> : <value>). Locate the conditional GOTO whose
// target is the fallback or the value assignment so the caller can rebuild the
// full conditional regardless of which arm the branch jumps to. Returns
// instructions.end() when no such branch exists.
static goto_programt::const_targett find_short_circuit_guard(
  const goto_programt &function_body,
  const goto_programt::const_targett &fallback_loc,
  const goto_programt::const_targett &value_loc,
  const goto_programt::const_targett &assume_location)
{
  forall_goto_program_instructions (it, function_body)
  {
    if (it == assume_location)
      break;
    if (it->is_goto() && !is_true(it->guard))
    {
      for (const auto &t : it->targets)
        if (t == fallback_loc || t == value_loc)
          return it;
    }
  }
  return function_body.instructions.end();
}
  ...
  const goto_programt::const_targett &assume_location)
{
  ...
  if (is_symbol2t(expr))
  {
      ...
      sym_name.find("___ESBMC_old") == std::string::npos)
    {
      ...
      // Multiple assignments - Clang's short-circuit lowering of `<guard> &&
      // <value>` (resp. `<guard> || <value>`):
      //   T = NONDET               (initialization)
      //   IF <guard> GOTO L_short
      //   T = <value>              (guard held / short circuit not taken)
      //   GOTO L_end
      //   L_short: T = <const>     (const = false/0 for &&, true/1 for ||)
      //   L_end:
      // which is exactly `T == (<guard> ? <const> : <value>)`. The leading
      // conjuncts of a 3+-term chain live in <guard> (as a reference to a nested
      ...
      for (const auto &assign : assignments)
      {
        ...
        // The short-circuit fallback: a trivial constant 0/1/false/true.
        if (is_constant_bool2t(assign.value))
        {
          fallback = assign.value;
          fallback_loc = assign.location;
          continue;
        }
        if (is_constant_int2t(assign.value))
        {
          const auto &val = to_constant_int2t(assign.value).value;
          if (val == 0 || val == 1)
          {
            fallback = assign.value;
            fallback_loc = assign.location;
        ...
        // The value assigned when the short circuit is not taken.
        if (is_nil_expr(value))
        {
          value = assign.value;
          value_loc = assign.location;
        }
      }
      ...
      expr2tc inlined_value =
        inline_temporary_variables(value, function_body, value_loc);

      // Recover the branch guard so the leading conjuncts are preserved. Fall
      // back to the value alone when the control-flow shape doesn't match (no
      // constant fallback, no guarding branch, or an ill-typed conditional).
      goto_programt::const_targett guard_loc =
        is_nil_expr(fallback)
          ? function_body.instructions.end()
          : find_short_circuit_guard(
              function_body, fallback_loc, value_loc, assume_location);
      if (
        guard_loc == function_body.instructions.end() ||
        fallback->type->type_id != expr->type->type_id ||
        inlined_value->type->type_id != expr->type->type_id)
        return inlined_value;

      expr2tc inlined_guard =
        inline_temporary_variables(guard_loc->guard, function_body, guard_loc);

      // The guard may branch to either arm: `IF guard GOTO L_short` (jumps to
      // the constant) or `IF guard GOTO L_value` (jumps to the value). Rebuild
      // `guard ? <target-arm> : <fall-through-arm>` accordingly.
      bool targets_fallback = false;
      for (const auto &t : guard_loc->targets)
        if (t == fallback_loc)
        {
          targets_fallback = true;
          break;
        }
      return targets_fallback
               ? if2tc(expr->type, inlined_guard, fallback, inlined_value)
               : if2tc(expr->type, inlined_guard, inlined_value, fallback);
    }
  }
