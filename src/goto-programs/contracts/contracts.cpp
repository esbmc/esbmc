...
expr2tc code_contractst::extract_ensures_clause(const symbolt &contract_symbol)
{
  ...
  // For now, we store both requires and ensures in the value
  // TODO: Implement proper separation if needed
  return gen_true_expr();
}
  ...
  const locationt &location)
{
  ...
  for (const auto &target : targets)
  {
    ...
    t->location = location;
    t->location.comment("contract assigns havoc");
  }
}
...
void code_contractst::enforce_contracts(const std::set<std::string> &to_enforce)
{
  ...
  {
    // Skip compiler-generated functions (destructors, etc.)
