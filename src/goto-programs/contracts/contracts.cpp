  ...
  const goto_programt::const_targett &assume_location)
{
  ...
  if (is_symbol2t(expr))
  {
    ...
    if (sym_name.find("$tmp") != std::string::npos)
    {
      // Search backwards from assume_location to find the assignment to this variable
      goto_programt::const_targett search_it = assume_location;
      while (search_it != function_body.instructions.begin())
      {
        ...
        if (search_it->is_assign() && is_code_assign2t(search_it->code))
        {
          ...
          if (is_symbol2t(assign.target))
          {
            ...
            if (target_sym.thename == sym.thename)
            {
