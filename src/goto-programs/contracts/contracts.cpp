  ...
  const std::map<irep_idt, param_extentt> &param_extents)
{
  ...
  for (const auto &t : classified.pointer_targets)
  {
    ...
    std::string didx_sym_name =
      "__ESBMC_frame_arr_didx_" + func_name + "_" + arr_name + "_" + cnt_str;

    symbolt didx_obj;
    didx_obj.name = didx_sym_name;
    didx_obj.id = didx_sym_name;
    set_symbol_type(didx_obj, j_type);
    ...
    expr2tc declared_idx = symbol2tc(j_type, didx_added->id);

    goto_programt::targett didx_decl = wrapper.add_instruction(DECL);
    didx_decl->code = code_decl2tc(j_type, didx_added->id);
    didx_decl->location = location;
    didx_decl->location.comment("frame: array-elem declared index (Phase 2B)");

    goto_programt::targett didx_assign = wrapper.add_instruction(ASSIGN);
    didx_assign->code = code_assign2tc(declared_idx, idx_expr);
    ...
    entry.declared_idx = declared_idx;
