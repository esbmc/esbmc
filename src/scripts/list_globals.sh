...
objdump -t $1 | awk '{if ($2 == "g" && $3 == "O") print $6;}' | c++filt | grep -v '^irept::.*$' | grep -v '^exprt::.*$' | grep -v '^typet::.*$' | grep -v '.*::field_names$' | sort
...
objdump -t $1 | awk '{if ($2 == "l" && $3 == "O" && ($4 == ".bss" || $4 == ".data")) print $6;}' | c++filt | grep -v 'std::__ioinit' |  sort
