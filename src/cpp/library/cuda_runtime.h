...
cudaEventCreate(cudaEvent_t *event, unsigned int flags)
{
...
cudaMallocHost(void **ptr, size_t size, unsigned int flags)
{
...
template <class T>
__inline__ __host__ cudaError_t
cudaHostAlloc(T **ptr, size_t size, unsigned int flags)
{
...
template <class T>
__inline__ __host__ cudaError_t
cudaHostGetDevicePointer(T **pDevice, void *pHost, unsigned int flags)
{
...
template <class T>
__inline__ __host__ cudaError_t
cudaMallocHost(T **ptr, size_t size, unsigned int flags = 0)
{
...
template <class T>
__inline__ __host__ cudaError_t
cudaMallocPitch(T **devPtr, size_t *pitch, size_t width, size_t height)
{
 ...
 */
template <class T>
__inline__ __host__ cudaError_t cudaGetSymbolSize(size_t *size, const T &symbol)
{
...
{
  return cudaOccupancyMaxPotentialBlockSizeVariableSMem(
    minGridSize,
    blockSize,
    func,
  ...
  int blockSizeLimit = 0)
{
      ...
      blockSizeToTryAligned -= granularity)
  {
    ...
    status = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &occupancyInBlocks, func, blockSizeToTry, dynamicSMemSize);
