...
*/
__device__
	float 	powf_gpu (float x, float y); //powf_gpu para não confundir com pow definida por c++ em math.h
...
__device__
	float 	powf_gpu (float a, float b){
...
float 	__powf_gpu ( float  x, float  y ){
	   ...
	   if (y < 0)
		   return 1 / __powf_gpu(x, -y); /// verificar se funciona, por causa do pow de C++
	   else {
		   for (int i = 0; i < y; ++i )
			   result *= x;
