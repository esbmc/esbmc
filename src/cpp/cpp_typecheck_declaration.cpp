  ...
  codet &code)
{
  ...
  // do scoping
  symbolt union_symbol=context.get_unordered_symbols()[follow(symbol.type).name()];
  ...
  context.get_unordered_symbols()[union_symbol.name].type.set(
  ...
  cpp_declarationt &declaration)
{
  ...
  Forall_cpp_declarators(it, declaration)
  {
    ...
    if(declaration.find("#template").is_not_nil())
    {
      ...
      symbol.type.set("#template_arguments", declaration.find("#template_arguments"));
    }
