...
public:
  ...
  /**
   * Adjust the body of a lambda call operator.
   * The AST generated by Clang refers to the captures, while it should instead refer to the captured fields in the closure type.
   *
   * @param symbol The symbol for which the call operator is adjusted.
   */
  void adjust_lambda_call_operator(symbolt &symbolt);

  /**
   * Replace lambda capture fields in `op` if it refers to a captured identifier.
   * `lambda_capture_fields` is a map of the lambda captures, where the key is
   * the identifier of the captured field and the value is an expression that refers to the field in the closure type.
   * @param lambda_capture_fields map of captured field identifiers to closure field expressions
   * @param op expression to replace lambda captures in
   */
  void
  replace_lambda_capture_fields(const irept &lambda_capture_fields, exprt &op);
};
