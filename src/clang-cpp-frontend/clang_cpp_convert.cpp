  ...
  struct_union_typet &type)
{
  ...
  if(auto cxxrd = llvm::dyn_cast<clang::CXXRecordDecl>(&rd))
  {
    ...
    for(const auto &decl : cxxrd->bases())
    {
      ...
      auto const *base = decl.getType().getTypePtr()->getAsCXXRecordDecl();
      assert(base != nullptr);
...
bool clang_cpp_convertert::get_expr(
  ...
  switch(stmt.getStmtClass())
  {
    ...
    case clang::Stmt::CXXNewExprClass:
    {
      const clang::CXXNewExpr &ne =
        static_cast<const clang::CXXNewExpr &>(stmt);
  ...
  exprt &new_expr)
{
  ...
  {
    auto * redecl =
      llvm::dyn_cast<SpecializationDecl>(redecl_with_bad_type);
    if (!redecl)
    {
      assert(llvm::isa<clang::CXXRecordDecl>(redecl_with_bad_type)
