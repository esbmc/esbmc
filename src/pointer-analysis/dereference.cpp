  ...
  modet mode)
{
  ...
  modulus2tc mod(offset->type, offset, subtype_sz);
  simplify(mod);
  ...
  const guardt &guard)
{
  ...
  // size.
  constant_int2tc access_size_e(size_type2(), BigInt(access_size));
  add2tc upper_byte(size_type2(), unsigned_offset, access_size_e);
