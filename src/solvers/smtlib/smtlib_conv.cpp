...
{
  // A caller-supplied logic wins; otherwise pick one from the encoding,
  // dropping the QF_ prefix when the problem carries quantifiers.
  std::string used_logic = logic;
  if (used_logic.empty())
  {
    const bool has_quantifiers = options.get_bool_option("has-quantifiers");
    if (options.get_bool_option("int-encoding"))
      used_logic = has_quantifiers ? "AUFLIRA" : "QF_AUFLIRA";
    else if (options.get_bool_option("floatbv"))
      used_logic = has_quantifiers ? "AUFBV" : "QF_AUFBV";
    else
      used_logic = has_quantifiers ? "AUFBVFP" : "QF_AUFBVFP";
  }
