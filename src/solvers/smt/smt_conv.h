...
public:
  ...
  /** Convert a byte_update2tc with constant offset in integer mode,
   *  using mathematical operations to simulate bit manipulation. */
  expr2tc convert_byte_update_int_mode_constant_expr(
    const byte_update2t &data,
    expr2tc source,
    expr2tc offs,
    expr2tc update_value,
    unsigned int src_width);
  /** Convert a byte_update2tc with non-constant offset in integer mode,
   *  using conditional expressions for all possible byte positions. */
  expr2tc convert_byte_update_int_mode_non_constant_expr(
    const byte_update2t &data,
    expr2tc source,
    expr2tc offs,
    expr2tc update_value,
