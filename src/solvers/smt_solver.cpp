...
smt_astt smt_solver_baset::convert_ieee_arith_2op(const expr2tc &expr)
{
  ...
  /* Camada exposes add/mul/rem directly; sub and div keep ESBMC's wrappers,
   * which handle the cases camada leaves to the caller. */
  switch (expr->expr_id)
  {
  case expr2t::ieee_add_id:
    return solver->mkFPAdd(side_1, side_2, rm);
  case expr2t::ieee_sub_id:
    return mk_smt_fpbv_sub(side_1, side_2, rm);
  case expr2t::ieee_mul_id:
    return solver->mkFPMul(side_1, side_2, rm);
  case expr2t::ieee_div_id:
    return mk_smt_fpbv_div(side_1, side_2, rm);
  default:
    assert(expr->expr_id == expr2t::ieee_rem_id);
...
smt_astt smt_solver_baset::convert_array_store(const expr2tc &expr)
{
    ...
    !to_array_type(with.type).size_is_infinite)
  {
    ...
    {
      /* No bool-to-bitvector cast here: camada's backends take bools in
         arrays directly, which is why convert_array_of()'s own workaround
         is disabled. */
      ...
      for (const flat_storet &s : stores)
      {
        expr2tc idx = s.index;
        simplify(idx);
        src = ast_update(src, convert_ast(s.value), 0, idx);
      }
      return src;
    }
