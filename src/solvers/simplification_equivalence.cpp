  ...
  std::string *witness)
{
  ...
  {
    if (!ctx)
      /* After the camada refactor create_solver takes no solver-name argument
       * -- camada dispatches the backend itself -- and returns a unique_ptr
       * rather than a raw pointer, so assign instead of reset(). */
      ctx = create_solver(ns, options);
