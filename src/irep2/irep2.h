...
public:
   ...
   *  For use in hash data structures; produces a size_t digest via
   *  hash_combine over the node's fields (formerly a crc32).
...
// stops counting them as "missed". The excluded members are the non-reflected
// source `locationt`s carried by many code/if kinds (esbmc/esbmc#4715) — the
// V.4 structured-CF kinds carry a `location`, code_block2t also carries an
// `end_location`, and if2t (an expression) carries the ternary position — plus
// the per-loop `pragma_unroll_count` on the loop kinds. Source locations must
// travel with the statement for goto_convert, but must not enter value
// identity (matching how a goto instructiont stores its locationt separately
// from its IREP2 code). Kinds without such members get 0.
