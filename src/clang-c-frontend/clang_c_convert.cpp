  ...
  bool complete)
{
   ...
   */
  if (!rd.isCompleteDefinition() || !complete)
    return false;
  ...
  exprt &new_expr)
{
  ...
  // We skip those functions as they are unused anyway and the rest of esbmc can't handle undeduced types.
  if (fd.getReturnType()->isUndeducedType() && !fd.isUsed(true))
  {
    return false;
  }
  ...
  bool complete)
{
  ...
  case clang::Type::Enum:
  {
    ...
    else if (get_type(q_type, new_type, true))
      return true;
  ...
  case clang::Type::Auto:
  {
    ...
    assert(at.isDeduced());
    if (get_type(at.desugar(), new_type, true))
      return true;
      ...
      if (!is_member_decl_static(member))
      {
        ...
        if (cxxrd /*&& !cxxrd->isCLike()*/)
        {
          ...
          cpp_data_object::get_data_object_symbol_type(
            id, data_object_symbol_type);
          exprt data_object_base = member_exprt(
            base,
            name + cpp_data_object::data_object_suffix,
            data_object_symbol_type);
          base.swap(data_object_base);
        }
...
bool clang_c_convertert::get_decl_ref(const clang::Decl &d, exprt &new_expr)
{
  ...
  if (const auto *fd = llvm::dyn_cast<clang::FieldDecl>(&d))
  {
    ...
    if (cxxrd)
    {
      ...
      typet this_type = pointer_typet(class_type);
      exprt this_expr;
