...
static const Type *getFullyQualifiedTemplateType(
  const ASTContext &Ctx,
  const Type *TypePtr,
  bool WithGlobalNsPrefix)
{
  ...
  bool WithGlobalNsPrefix)
{
  ...
  if(Outer && !(OuterNS && OuterNS->isAnonymousNamespace()))
  {
    ...
    {
      if(ClassTemplateDecl *ClassTempl = CxxDecl->getDescribedClassTemplate())
      {
  ...
  bool WithGlobalNsPrefix)
{
  ...
  if(
    isa<const TemplateSpecializationType>(QT.getTypePtr()) ||
