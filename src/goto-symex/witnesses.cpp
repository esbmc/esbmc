  ...
  const goto_tracet &goto_trace)
{
  ...
  // Extract function name and module name from goto_trace
  std::string function_name = "unknown_function";
  std::string module_name = "module";
  std::string source_file = "";
  std::string violation_comment = "";
  std::string violation_property = "";

  // Find the violated property and function name from goto_trace
  for (const auto &step : goto_trace.steps)
  {
    if (step.type == goto_trace_stept::ASSERT && !step.guard)
    {
      // Found the violation
      if (step.pc->location.is_not_nil())
      {
        function_name = step.pc->location.function().as_string();
        source_file = step.pc->location.file().as_string();

        // Extract module name from file path (remove .py extension)
        if (!source_file.empty())
        {
          ...
          {
            module_name =
              source_file.substr(last_slash + 1, last_dot - last_slash - 1);
          }
...
Violation: )"
              << violation_comment << R"(
Property: )"
              << violation_property << R"(
"""
import pytest
)";

  // Add import statement if we have a module name
  if (function_name != "unknown_function" && function_name != "__ESBMC_main" &&
      function_name != "python_init" && function_name != "python_user_main")
  {
    pytest_file << "from " << module_name << " import " << function_name
                << "\n";
  }
  ...
  // Start test function
  pytest_file << "def test_" << function_name << "_counterexample():\n";
  pytest_file << R"(    """
  ...
  // Collect input variables and their values
  // We need to track variables that appear in the trace
  std::map<std::string, expr2tc> input_values;
  ...
  for (auto const &SSA_step : target.SSA_steps)
  {
    ...
    if (SSA_step.is_assignment() && !is_nil_expr(SSA_step.lhs))
    {
      if (is_symbol2t(SSA_step.lhs))
      {
        ...
        // Python input variables typically don't have special prefixes
        // Skip SSA variables (containing !), return values, and internal vars
        if (var_name.find('!') != std::string::npos ||
            var_name.find('$') != std::string::npos ||
            var_name.find("return_value") != std::string::npos ||
            var_name.find("__ESBMC") != std::string::npos ||
            var_name.find("python_") == 0 || var_name.find("tmp") == 0 ||
            var_name.find("guard#") != std::string::npos)
        {
          continue;
        }

        // Get the value from SMT solver
        auto value = smt_conv.get(SSA_step.lhs);

        if (!is_nil_expr(value) && seen_vars.find(var_name) == seen_vars.end())
        {
  ...
  // If we didn't find variables from SSA, try goto_trace
      ...
      if (step.type == goto_trace_stept::ASSIGNMENT)
      {
        if (!is_nil_expr(step.lhs) && is_symbol2t(step.lhs))
        {
          ...
          if (!is_nil_expr(step.value) &&
              seen_vars.find(var_name) == seen_vars.end())
          {
            input_values[var_name] = step.value;
            seen_vars.insert(var_name);
          }
        }
      }
  ...
  pytest_file << "    # Input values from counterexample\n";

  for (const auto &[var_name, value] : input_values)
  {
    pytest_file << "    " << var_name << " = ";

    // Format value based on type (照抄C的类型转换逻辑)
    ...
    else if (is_constant_floatbv2t(value))
    {
      ...
      pytest_file << float_str;
    }
    else if (is_constant_bool2t(value))
    {
      pytest_file << (to_constant_bool2t(value).value ? "True" : "False");
    }
    else if (is_constant_string2t(value))
    {
      pytest_file << "\"" << to_constant_string2t(value).value.as_string()
                  << "\"";
    }
    else
    {
      // Fallback: use from_expr
      pytest_file << from_expr(ns, "", value);
    }
  ...
  if (input_values.empty())
  {
    pytest_file << "    # No input variables found in counterexample\n";
    pytest_file << "    pass\n";
  }
  else
  {
    // Generate function call with exception handling
    pytest_file << "\n    # This call should reproduce the bug\n";

    // Determine expected exception type from violation comment
    std::string exception_type = "Exception";
    if (violation_comment.find("division by zero") != std::string::npos)
    {
      exception_type = "ZeroDivisionError";
    }
    else if (violation_comment.find("array bounds") != std::string::npos ||
             violation_comment.find("out of bounds") != std::string::npos)
    {
      exception_type = "IndexError";
    }
    else if (violation_comment.find("assertion") != std::string::npos)
    {
      exception_type = "AssertionError";
    }

    pytest_file << "    with pytest.raises(" << exception_type << "):\n";
    pytest_file << "        result = " << function_name << "(";

    // Build argument list
    bool first = true;
    for (const auto &[var_name, _] : input_values)
    {
      if (!first)
        pytest_file << ", ";
      pytest_file << var_name;
  ...
  // Add a placeholder for manual test
  pytest_file << "def test_" << function_name << "_manual():\n";
  pytest_file << R"(    """
  ...
  log_status("Generated pytest test file: {}", file_name);
}