    ...
    const goto_tracet & goto_trace)
{
      ...
      it != goto_trace.steps.end(); it++)
  {
    ...
    std::string::size_type find_bt = it->pc->location.to_string().find(
        "built-in", 0);
    std::string::size_type find_lib = it->pc->location.to_string().find(
        "library", 0);
    ...
    if (it->lhs->type->type_id != it->lhs->type->array_id)
    {
      ...
      /* TODO check if is an union, struct or dynamic attr. However, we need more details of the validation tools */
