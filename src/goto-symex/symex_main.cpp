...
void goto_symext::loop_inv_provert::do_base_cases(goto_symex_statet *cur_state)
{
  ...
  for (auto &inv : inv_instruction.invariants)
  {
    ...
    expr2tc question = equality2tc(gen_false_expr(), new_expr);
    // check whether the assertion holds
    target->assert_top_formula();
    target->assert_fact(question);

    log_status(
      "Attempting to prove base case: {}", from_expr(ns, "", new_expr));
  ...
  log_status(
    "Finished with base cases for loop at location: {}\n", loop_location);

  if (!invs_passed_base_case.size())
  {
    log_error(
      "For loop at location: {}, no potential invariant passed the base case "
      "check. Aborting",
      loop_location);
    abort();
  }
...
void goto_symext::loop_inv_provert::do_step_cases()
{
  ...
  while (solved_in_previous_round)
  {
    ...
    for (int i = 0; i < induction_hypotheses.size(); i++)
    {
      ...
      {
        expr2tc hyp = equality2tc(gen_true_expr(), induction_hypotheses[i]);
        expr2tc conc = equality2tc(gen_false_expr(), conclusions[i]);
        target->assert_top_formula();
        target->assert_fact(hyp);
        target->assert_fact(conc);
        for (auto &hyp : proved_invs)
        {
          expr2tc fact = equality2tc(gen_true_expr(), hyp);
          target->assert_fact(fact);
        }

        log_status(
          "Attempting to prove step case: {}",
          from_expr(ns, "", conclusions[i]));
  ...
  {
    log_error(
      "For loop at location: {}, no potential invariant passed the step case "
      "check. Aborting",
      loop_location);
    abort();
  }
  log_status(
    "Finished with step cases for loop at location: {}\n", loop_location);
...
void goto_symext::loop_inv_provert::get_conclusions(
...
void goto_symext::symex_invariant()
{
  ...
  loop_inv_provers.push_back(
    loop_inv_provert(ns, instruction, target, loop_loc));
