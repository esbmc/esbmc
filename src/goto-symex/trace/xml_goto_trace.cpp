...
void convert(const namespacet &ns, const goto_tracet &goto_trace, xmlt &xml)
{
  ...
  for (const auto &step : goto_trace.steps)
  {
    ...
    case goto_trace_stept::ASSERT:
      if (!step.guard)
      {
        ...
        xml_failure.new_element("reason").data =
          xmlt::escape(id2string(step.comment));
    ...
    case goto_trace_stept::ASSIGNMENT:
    {
      ...
      xml_assignment.new_element("identifier").data =
        xmlt::escape(id2string(identifier));
      xml_assignment.new_element("base_name").data =
        xmlt::escape(id2string(base_name));
      xml_assignment.new_element("display_name").data =
        xmlt::escape(id2string(display_name));
      xml_assignment.new_element("value").data =
        xmlt::escape(id2string(value_string));
      xml_assignment.new_element("type").data =
        xmlt::escape(id2string(type_string));
      ...
      if (location != previous_location)
      {
        if (xml_location.name != "")
        {
          ...
          xml_location_only.new_element("step_nr").data =
            i2string(step.step_nr);
          xml_location_only.new_element("thread").data =
            i2string(step.thread_nr);
