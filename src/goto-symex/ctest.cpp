...
// Every file this generator writes carries this on its first line, so an
// existing file without it belongs to the user (github #6214).
static const char ESBMC_GENERATED_MARKER[] = "Auto-generated by ESBMC";
...
// it to lose.
static overwrite_verdict classify_existing(const std::filesystem::path &path)
{
...
// than either writing all of it or none of it.
static bool can_write_all(const std::vector<std::filesystem::path> &paths)
{
  ...
  const std::vector<std::string> &test_files)
{
  ...
  return can_write_all(all_outputs(out_dir, test_files));
}
