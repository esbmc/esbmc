  ...
  const std::vector<std::string> &test_files)
{
  ...
  for (size_t i = 0; i < target_names.size(); ++i)
  {
    ...
    cmake << "add_test(NAME " << target_names[i] << " COMMAND "
          << target_names[i] << ")\n";
    // Force-include esbmc_verifier.h so the C source file gets the
    // __VERIFIER_* declarations it needs.
...
// Write CMakeLists.txt for a C++ project.
// Generates esbmc_verifier.h(as same as C version)
static void write_cpp_cmake(
  const std::string &project_name,
  const std::string &src_file,
  const std::vector<std::string> &target_names,
  const std::vector<std::string> &test_files)
{
  ...
  cmake << "# Auto-generated by ESBMC " << ESBMC_VERSION << "\n";
  cmake << "cmake_minimum_required(VERSION 3.10)\n";
  cmake << "project(" << project_name << " CXX)\n\n";
  cmake << "enable_testing()\n\n";
  cmake << "option(ENABLE_COVERAGE \"Enable coverage reporting\" OFF)\n";
  cmake
  ...
  for (size_t i = 0; i < target_names.size(); ++i)
  {
    ...
    cmake << "add_test(NAME " << target_names[i] << " COMMAND "
          << target_names[i] << ")\n";
    // Force-include esbmc_verifier.h so the C++ source file gets the extern
    // "C" declarations as same as C version.
    cmake << "target_compile_options(" << target_names[i]
          << " PRIVATE -include ${CMAKE_CURRENT_SOURCE_DIR}/esbmc_verifier.h)"
             "\n\n";
  }
...
void ctest_generator::generate() const
{
  ...
  for (size_t i = 0; i < unique_cases.size(); ++i)
  {
    ...
    if (cpp_mode)
      write_cpp_test_file(
        test_file, type_values, c_types, i + 1, unique_cases.size());
    else
      write_c_test_file(
        test_file, type_values, c_types, i + 1, unique_cases.size());
  }
  ...
  if (cpp_mode)
    write_cpp_cmake(
      "ESBMCGeneratedTests", source_file, target_names, test_file_names);
  else
    write_c_cmake(
      "ESBMCGeneratedTests", source_file, target_names, test_file_names);
  ...
  const namespacet &ns)
{
  ...
  if (cpp_mode)
  {
    ...
    write_cpp_cmake(
      "ESBMCGeneratedTest", src_file, {"test_case"}, {test_file_name});
  }
  else
  {
