...
def annotate_return(func_node: ast.FunctionDef, ret_type):
    ...
    if isinstance(ret_type, (Unknown, AnyType, Bottom)):
        return

    if isinstance(ret_type, UnionType):
        supported_types = (BoolType, IntType, FloatType)

        for member in ret_type.members:
            if not isinstance(member, supported_types):
                return

    if isinstance(ret_type, TupleType):
        for member in ret_type.elems:
            if isinstance(member, (Unknown, AnyType, Bottom)):
                return
...
def annotate_function_with_env_and_signatures(
    ...
    param_types, ret_type = derive_param_and_return_types(
        func_node,
        out_envs,
        return_types,
        context,
    ...
    for stmt in func_node.body:
        ...
        container = stmt.value.value

        if not isinstance(container, ast.Name):
            continue

        if container.id not in parameter_names:
            continue

        print(
            "[ANNOTATE DEBUG] removing inferred local:",
            target.id,
            "because it comes from parameter",
            container.id,
        )

        merged.pop(target.id, None)

    reassigned = collect_reassigned_names(func_node)
...
def annotate_ast(ast_node, opts=None):
    ...
    collect_function_argument_types(ast_node, context)
