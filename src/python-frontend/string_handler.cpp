  ...
  const locationt &location)
{
  ...
  exprt haystack_copy = str_expr;
  exprt needle_copy = sub_expr;

  exprt haystack = ensure_null_terminated_string(haystack_copy);
  exprt needle = ensure_null_terminated_string(needle_copy);

  exprt haystack_ptr =
    haystack.type().is_pointer() ? haystack : get_array_base_address(haystack);
  exprt needle_ptr =
    needle.type().is_pointer() ? needle : get_array_base_address(needle);

  side_effect_expr_function_callt call;
  call.function() = symbol_exprt(func_symbol_id, code_typet());
  call.arguments().push_back(haystack_ptr);
  call.arguments().push_back(needle_ptr);
  call.type() = int_type();
  call.location() = location;
