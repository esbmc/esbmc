...
- **Data Structures**: Supports operations on Python's built-in data structures, including lists, strings, and tuples, with features such as concatenation and bounds checks.
  ...
  - **String Operations**:
    ...
    - **strip() method**: Removes leading and trailing whitespace characters (e.g., `"  hello  ".strip()` returns `"hello"`). Currently supports only the no-argument form that trims the default whitespace set.
    - **count() method**: Counts non-overlapping occurrences of a substring (e.g., `"team".count("e")` returns `1`). The optional `start`/`end` parameters are not yet supported.
