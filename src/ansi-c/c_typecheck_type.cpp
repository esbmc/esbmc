...
void c_typecheck_baset::typecheck_code_type(code_typet &type)
{
  ...
  else if(arguments.size() == 1 && arguments[0].type().id() == "empty")
  {
  ...
  else
  {
    for(auto &argument : code_type.arguments())
    {
      ...
      if(type.id() == "KnR")
      {
        ...
        // may be renamed
        {
          id_replace_mapt::const_iterator m_it =
            id_replace_map.find(identifier);
...
void c_typecheck_baset::typecheck_array_type(array_typet &type)
{
  ...
  if(size.is_constant())
  {
    ...
    {
      err_location(size);
      str << "failed to convert constant: " << size.pretty();
      throw 0;
    }
...
void c_typecheck_baset::typecheck_c_bit_field_type(typet &type)
{
  ...
  if(
    base_type.id() != "signedbv" && base_type.id() != "unsignedbv" &&
    base_type.id() != "c_enum")
  {
    err_location(type);
    str << "Bit-field with non-integer type: " << to_string(base_type);
    throw 0;
  }
