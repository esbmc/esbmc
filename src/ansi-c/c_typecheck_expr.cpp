...
void c_typecheck_baset::typecheck_expr_typecast(exprt &expr)
{
  ...
  {
    err_location(expr);
    str << "type cast to `" << to_string(expr.type()) << "' not permitted";
    throw 0;
  }
  ...
  {
    err_location(expr);
    str << "type cast from `" << to_string(op_type) << "' not permitted";
    throw 0;
  }
...
void c_typecheck_baset::typecheck_expr_rel(exprt &expr)
{
  ...
  err_location(expr);
  str << "operator `" << expr.id_string() << "' not defined for types `"
...
void c_typecheck_baset::typecheck_expr_trinary(exprt &expr)
{
  ...
  err_location(expr);
  str << "operator ?: not defined for types `" << to_string(o_type1)
      << "' and `" << to_string(o_type2) << "'";
  throw 0;
}
...
void c_typecheck_baset::typecheck_expr_dereference(exprt &expr)
{
  ...
  {
    err_location(expr);
    str << "operand of unary * `" << to_string(op) << "' is not a pointer";
    throw 0;
  }
...
void c_typecheck_baset::typecheck_expr_side_effect(side_effect_exprt &expr)
{
    ...
    statement == "postincrement" || statement == "postdecrement")
  {
    ...
    {
      err_location(expr);
      str << "operator `" << statement << "' not defined for type `"
          << to_string(type0) << "'";
      throw 0;
    }
  }
  ...
  side_effect_expr_function_callt &expr)
{
  ...
  else if(argument_types.size() != arguments.size())
  {
    ...
    str << "wrong number of arguments: "
        << "expected " << argument_types.size() << ", but got "
        << arguments.size();
    throw 0;
  }
...
void c_typecheck_baset::typecheck_expr_constant(exprt &expr [[maybe_unused]])
{
  // Do nothing
}
...
void c_typecheck_baset::typecheck_expr_unary_arithmetic(exprt &expr)
{
  ...
  err_location(expr);
  str << "operator `" << expr.id_string() << "' not defined for type `"
...
void c_typecheck_baset::typecheck_expr_pointer_arithmetic(exprt &expr)
{
  ...
  err_location(expr);
  str << "operator `" << expr.id_string() << "' not defined for types `"
      << to_string(type0) << "' and `" << to_string(type1) << "'";
  throw 0;
}
...
void c_typecheck_baset::typecheck_side_effect_assignment(exprt &expr)
{
  ...
  err_location(expr);
  str << "assignment `" << statement << "' not defined for types `"
      << to_string(o_type0) << "' and `" << to_string(o_type1) << "'";
...
void c_typecheck_baset::make_constant_rec(exprt &expr [[maybe_unused]])
{
}
