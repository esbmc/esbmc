...
static void init_variable(codet &dest, const symbolt &sym)
{
...
bool c_main(contextt &context, const std::string &standard_main)
{
    ...
    else if (arguments.size() == 2 || arguments.size() == 3)
    {
...

      {
        ...
        index_expr.copy_to_operands(
          symbol_expr(argv_symbol), symbol_expr(argc_symbol));
      ...
      if (arguments.size() == 3)
      {
        ...
        index_expr.copy_to_operands(
          symbol_expr(envp_symbol), symbol_expr(envp_size_symbol));
  ...
  thread_start_call.function() = symbol_exprt("__ESBMC_pthread_start_main_hook");
