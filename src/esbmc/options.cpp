...
const struct group_opt_templ all_cmd_options[] = {
  ...
  {"Transition system (experimental)",
   {{"ts-dump",
    ...
    {"ts-no-fallback",
     NULL,
     "Report UNKNOWN instead of falling back when the program is not a "
     "transition system"},
    {"ts-no-live-filter", NULL, "Keep every havocked variable as state"},
    {"ts-no-simple-path",
     NULL,
     "Do not add simple-path constraints in --ts-k-induction"},
    {"ts-fresh-solver",
     NULL,
     "With --ts-bmc, solve each bound as one formula in a new solver"},
    {"ts-push-pop",
     NULL,
     "Use push/pop instead of assumptions in --ts-bmc (cross-check)"}}},
  {"k-induction",
