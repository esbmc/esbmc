...
bool solidity_convertert::convert()
{
  ...
  // second round: populate linearizedBaseList & contract_namelist
  // this is to obtain the inheritance and contract name list
  index = 0;
  for (nlohmann::json::iterator itr = nodes.begin(); itr != nodes.end();
       ++itr, ++index)
  {
  ...
  for (const auto &sym : exportedSymbolsList)
  {
    ...
    {
      contract_namelist.insert(c_name);
    }
  }
  assert(!contract_namelist.empty());
       ...
       ++itr, ++index)
  {
    ...
    if (node_type == "ContractDefinition") // rule source-unit
    {
      ...
      // add built-in state properties, e.g. balance
      // or auxiliary vars e.g. address, is_trusted
  ...
  if (t.get("#sol_type") == "CONTRACT")
  {
         ...
         i++)
    {
      ...
      if (c_name == tgt_c_name)
      {
        ...
        if (
          decl.contains("contractKind") && decl["contractKind"] == "interface")
        {
          is_interface = true;
          // serach inheritance
          for (auto j = linearizedBaseList.begin();
               j != linearizedBaseList.end();
               j++)
          {
...
bool solidity_convertert::get_struct_class(const nlohmann::json &struct_def)
{
  ...
  log_debug("solidity", "Finish parsing struct {}", name);
  return false;
}
  ...
  struct_typet &type)
{
  ...
  if (
    ...
    comp.set("#is_payable", true);
  else
    comp.set("#is_payable", false);
...
{
  log_debug("solidity", "get_contract_definition");

  auto tmp_functionDecl = current_functionDecl;
  auto tmp_functionName = current_functionName;
  auto tmp_current_contractName = current_contractName;
  auto tmp_current_functionName = current_functionName;
  auto tmp_current_functionDecl = current_functionDecl;
  auto tmp_current_typeName = current_typeName;
  auto tmp_current_forStmt = current_forStmt;
  auto tmp_current_backBlockDecl = current_backBlockDecl;
  auto tmp_global_scope_id = global_scope_id;
  auto tmp_map_init_block = map_init_block;
  auto tmp_initializers = initializers;
  auto tmp_ctor_modifier = ctor_modifier;
  auto tmp_base_contracts = base_contracts;
  ...
  current_functionDecl = tmp_functionDecl;
  current_functionName = tmp_functionName;
  current_contractName = tmp_current_contractName;
  current_functionName = tmp_current_functionName;
  current_functionDecl = tmp_current_functionDecl;
  current_typeName = tmp_current_typeName;
  current_forStmt = tmp_current_forStmt;
  current_backBlockDecl = tmp_current_backBlockDecl;
  global_scope_id = tmp_global_scope_id;
  map_init_block = tmp_map_init_block;
  initializers = tmp_initializers;
  ctor_modifier = tmp_ctor_modifier;
  base_contracts = tmp_base_contracts;
  ...
  std::string ctor_id)
{
  ...
  // for auxilary sol_cname_array
  // to store a mapping from address to current_contract_name;

  exprt _mapping;
  if (set_addr_cname_mapping(contract_name, base, _mapping))
    return true;
  convert_expression_to_code(_mapping);
  sym.value.operands().push_back(_mapping);
  ...
  const nlohmann::json &ast_node)
{
  ...
  // skip if we have already handled
  if (context.find_symbol(id) != nullptr)
  {
    current_functionDecl = old_functionDecl;
    current_functionName = old_functionName;
  ...
  exprt &param)
{
  ...
  typet param_type;
  assert(pd.contains("typeDescriptions"));
  ...
  log_debug("solidity", "\t@@@ parsing function params, got {}", id);
  ...
  exprt &new_expr)
{
  ...
  case SolidityGrammar::ExpressionT::DeclRefExprClass:
  {
    ...
    {
      // TODO: move this into get_esbmc_builtin_ref
      if (expr["name"] == "this")
      {
        assert(current_functionDecl);
  ...
  case SolidityGrammar::ExpressionT::CallExprClass:
  {
    ...
    if (
      ...
      callee_expr_json["nodeType"] == "MemberAccess")
    {
      SolidityGrammar::ExpressionT sub_type =
        SolidityGrammar::get_expression_t(callee_expr_json);
      switch (sub_type)
      {
      case SolidityGrammar::AddressMemberCall:
      {
        log_debug("solidity", "\t\t@@@ address member call");
        // e.g. addr.call
        // return type: expr["typeDescriptions"]
        nlohmann::json args = nullptr;
    ...
    if ((!get_esbmc_builtin_ref(callee_expr_json, new_expr) ||
      ...
      callee_expr_json["nodeType"] == "MemberAccess")
    {
      ...
      switch (sub_type)
      {
      case SolidityGrammar::ContractMemberCall:
      {
      ...
      default:
      {
        // other member calls are not funciton call
        log_error(
          "Unexpected or Unspported member calls, got {}",
          SolidityGrammar::expression_to_str(sub_type));
        return true;
      }
      }
      ...
      if (caller_expr_json.contains("referencedDeclaration"))
      {
        // instance.x();
      ...
      else
      {
        // Special
        // Base(addr).x();
        assert(caller_expr_json["kind"] == "typeConversion");
      ...
      // irep_idt __id  = base.type().subtype().identifier();
      assert(context.find_symbol(__id) != nullptr);

      std::string bs_c_name = context.find_symbol(__id)->name.as_string();
      if (bs_c_name.empty())
      {
        log_error(
          "cannot find base contract name\n{}\n{}",
          __id,
          symbol_expr(*context.find_symbol(__id)).to_string());
        return true;
      }
      external_transaction_verification_high(
        expr, base, copy, new_expr, bs_c_name);
  ...
  case SolidityGrammar::ExpressionT::NewExpression:
  {
    ...
    // gas--
    // Base tmp_x_i = new Base();
    // tmp_x_i.is_trusted = true;
    // Base &x = tmp_x_i;
    // msg.sender = old_msg.sender;

    // only handle in function declaraion
    if (current_functionDecl)
    {
      extend_extcall_modelling(current_contractName, call.location());

      symbolt sym;
      std::string name =
        "sol_tmp_" + call.name().as_string() + std::to_string(aux_counter);
      ++aux_counter;
      std::string bs_c_name =
      ...
      std::string id = "sol:@C@" + bs_c_name + "@" + name;

      get_default_symbol(sym, "C++", call.type(), name, id, call.location());
      sym.static_lifetime = false;
      sym.file_local = true;
      sym.type.set("#sol_type", "CONTRACT");
      ...
      // set tmp.is_trust = true;
      std::string t_id = "sol:@C@" + bs_c_name + "@is_trusted";
      ...
      exprt _mem =
        member_exprt(symbol_expr(added_symbol), trust.name(), trust.type());
      // do assignment
      exprt _assign = side_effect_exprt("assign", bool_type());
      _assign.copy_to_operands(_mem, true_exprt());
      current_frontBlockDecl.move_to_operands(_assign);

      // set initial value
      call = symbol_expr(added_symbol);
    }
    new_expr = call;
    break;
  }
  ...
  case SolidityGrammar::ExpressionT::AddressMemberCall:
  {
    ...
    {
      // case 2
      assert(is_low_level_call(mem_name));
  ...
  case SolidityGrammar::ExpressionT::TypeConversionExpression:
  {
    ...
    // 2. get target type
    if (get_type_description(conv_expr["typeDescriptions"], to_type))
      return true;

    // special: Base x = Base(_addr);
    // this is not an external calls as it will not call the ctor
    // thus we do not need the extend_extcall_modelling
    if (to_type.get("#sol_type").as_string() == "CONTRACT")
    {
      assert(
      ...
      //   tmp = new Base();
      //   tmp.addr = _addr;
      //   update_addr_obj(_addr, &tmp);
      // }
      // else
      // {
      //   tmp = *(Base *)get_obj(_addr);
      // }
      ...
      {
        std::string c_name =
          context.find_symbol(to_type.identifier())->name.as_string();
        std::string ctor_id;
        ...
        get_default_symbol(sym, "C++", to_type, name, id, call.location());
        ...
        get_library_function_call(
          ...
          call.location(),
          _get_addr_array_idx);
        _get_addr_array_idx.arguments().push_back(from_expr);
        ...
        exprt guard = exprt("=", bool_type());
        guard.operands().push_back(_get_addr_array_idx);
        guard.operands().push_back(neg_one);

        // populate body
        code_blockt if_body = code_blockt();

        // tmp = new Base();
        exprt _assign1 = side_effect_exprt("assign", to_type);
        ...
        // update_addr_obj(_addr, &tmp)
        side_effect_expr_function_callt _update_addr_obj;
        get_library_function_call(
          "update_addr_obj",
          "c:@F@update_addr_obj",
          empty_typet(),
          call.location(),
          _update_addr_obj);
        _update_addr_obj.arguments().push_back(from_expr);

        // get address: &tmp
        exprt ref_tmp = address_of_exprt(tmp);
        _update_addr_obj.arguments().push_back(ref_tmp);

        convert_expression_to_code(_update_addr_obj);
        if_body.move_to_operands(_update_addr_obj);

        // else
        code_blockt else_body = code_blockt();

        // *(Base *)get_obj(_addr);
        side_effect_expr_function_callt _get_obj;
        get_library_function_call(
          "get_obj", "c:@F@get_obj", empty_typet(), call.location(), _get_obj);
        _get_obj.arguments().push_back(from_expr);

        exprt _tycast = typecast_exprt(_get_obj, pointer_typet(to_type));
        exprt _def = dereference_exprt(_tycast, to_type);

        // tmp = *(Base *)get_obj(_addr);
        exprt _assign3 = side_effect_exprt("assign", to_type);
        _assign3.operands().push_back(tmp);
        _assign3.operands().push_back(_def);
        convert_expression_to_code(_assign3);
        else_body.move_to_operands(_assign3);

        codet if_expr("ifthenelse");
        if_expr.copy_to_operands(guard, if_body, else_body);
    ...
    // special: address x = address(new D(42)); address(this)
    else if (
      from_expr.type().get("#sol_type").as_string().find("ADDRESS") ==
        std::string::npos &&
      to_type.get("#sol_type").as_string().find("ADDRESS") != std::string::npos)
    {
      // aux.address
      // get addr
      irep_idt c_id;
      if (from_expr.is_member())
        c_id = from_expr.type().identifier();
      else
        c_id = from_expr.type().subtype().identifier();

      if (context.find_symbol(c_id) == nullptr)
      {
        log_error("cannot find the corresponding contract type\n{}", from_expr);
        return true;
      }
      ...
      exprt _address;
      get_addr_expr(bs_c_name, from_expr, _address);
      new_expr = _address;
      break;
    }

    // 3. generate the type casting expr
    convert_type_expr(ns, from_expr, to_type);
  ...
  std::string &contract_name)
{
  ...
  {
    log_debug("solidity", "\t\t@@@ contract member access");
  ...
  exprt &new_expr)
{
  ...
  if (context.find_symbol(func_id) == nullptr)
  {
    log_warning("function {} is not parsed yet", func_id);
    return true;
  }
  ...
  exprt &new_expr)
{
  ...
  if (blt_name == "require" || blt_name == "revert")
    ...
    name = blt_name;
  else if (blt_name == "this")
  {
    // e.g. address(this).balance;
    // => this->balance;
    assert(current_functionDecl);

    // get current function's this pointer
    exprt this_expr;
    std::string f_name, f_id;
    get_function_definition_name(*current_functionDecl, f_name, f_id);
    if (get_func_decl_this_ref(f_id, this_expr))
      return true;

    // set location
    locationt loc;
    get_location_from_decl(decl, loc);
    this_expr.location() = loc;
    if (current_functionDecl)
      this_expr.location().function(current_functionName);
  ...
  exprt &new_expr)
{
  ...
  else if (expr["nodeType"].get<std::string>() == "MemberAccess")
  {
    ...
    {
      log_error("cannot find the solidity builtin member, got {}", mem);
      return true;
    }
  }
  else
  ...
  typet &new_type)
{
  ...
  case SolidityGrammar::FunctionDeclRefT::FunctionProto:
  {
    ...
    assert(decl.contains("returnParameters"));
    const nlohmann::json &rtn_type = decl["returnParameters"];
  ...
  exprt &new_expr)
{
  ...
  comp.set_access("public"); // this should not affect the inheritance
  ...
  std::string &id)
{
    ...
    ast_node["kind"].get<std::string>() == "constructor")
  {
    ...
    ast_node["name"].get<std::string>().empty() && ast_node.contains("kind"))
  {
    // for receive, fallback;
    name = ast_node["kind"].get<std::string>();
    id = "sol:@C@" + contract_name + "@F@" + name + "#" +
         i2string(ast_node["id"].get<std::int16_t>());
  }
  else
  ...
  const std::string &contractName)
{
  ...
  */
  ...
  if (arbitrary_modelling(
        contractName, methods, symbol_expr(added_ctor_symbol), while_body))
    return true;
...
/*
  ...
  ...
*/
bool solidity_convertert::arbitrary_modelling(
  const std::string &contract_name,
  const struct_typet::componentst &methods,
  const exprt &base,
  codet &body)
  ...
  exprt &new_expr)
{
  ...
  // foreach contract
  for (const auto &cname : contract_namelist)
  {
...
/** 
  The default strategy for external call verification is that
  all external calls are regarded as trusted (unless it's unimplemented ,
  which  are treated as untrusted)
  ...
  to
    ...
    x.is_trust is initialized as nondet_bool but after the constructor initializaiton 
    the value would be fixed 
    @base: x
    @expr: func() 
    @bs_contract_name: Base
    @contract_name: current_contract_name
**/
void solidity_convertert::external_transaction_verification_high(
  const nlohmann::json &json,
  const exprt &base,
  const exprt &expr,
  exprt &new_expr,
  const std::string bs_contract_name)
{
  log_debug("solidity", "constructing high level external call verification");
  // check if it's preset
  // the annotation need to be set in the solidity co

  std::string contract_name;
  if (get_current_contract_name(json, contract_name))
    abort();

  // 1 trustd; 0 untrusted; -1 unset
  // priority: is_trust > trust_mode
  ...
  exprt trusted_expr = expr;
  exprt untrusted_expr;

  // construct sol_main_call
  //! TODO: handling the return value
  ...
  extend_extcall_modelling(contract_name, sol_loc);

  // cfg construction control
  if (trust_mode)
  {
  ...
  {
    exprt if_expr("if", empty_typet());
    if_expr.copy_to_operands(trust_flg, trusted_expr, untrusted_expr);
