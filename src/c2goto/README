...
2. The support code for loading the GOTO binaries and providing their contents
   ...
   and use pre-compiled versions of operational models in ESBMC in order to
...
3. In the `library` subdirectory some (but not all, see
   ...
   The same holds true for all other types and prototypes defined in `headers`.
   ...
   preprocessed. The list of include paths for this preprocessing is constructed
