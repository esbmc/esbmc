...
int ffs(int x)
{
    ...
    __ESBMC_ensures((__ESBMC_return_value == 0) || (((unsigned)x & (1u << (__ESBMC_return_value - 1))) != 0));
    __ESBMC_ensures((__ESBMC_return_value <= 1) || (((unsigned)x & ((1u << (__ESBMC_return_value - 1)) - 1u)) == 0));
    __ESBMC_assigns();
__ESBMC_HIDE:;
  if (x == 0)
    return 0;
