...
//Initialized considering overhead (time for data conditioning, DAC, ADC, etc...)
#define OVERHEAD 80
  ...
  int mode)
{
  ...
  for(j = 0; j < Nw - 1; j++)
  {
    ...
    {
      w[j] +=
        floatToFix(*b_ptr++ * x, delta, minFix, maxFix, mode); // *b_ptr++ * x;
      w[j] = floatToFix(w[j], delta, minFix, maxFix, mode);
    }
  }
