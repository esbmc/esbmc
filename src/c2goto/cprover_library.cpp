...
namespace
{
/// Ids the bundled libraries put into the context, and the subset the prune
/// removed. Both are process-wide because the libraries are loaded once.
std::unordered_set<irep_idt, irep_id_hash> library_ids;
std::unordered_set<irep_idt, irep_id_hash> pruned_ids;
} // namespace
  ...
  const goto_functionst *prelowered)
{
  ...
  std::multimap<irep_idt, irep_idt> deps;
  context.foreach_operand([&deps](const symbolt &s) {
    generate_symbol_deps(s.id, s.get_value(), deps);
    generate_symbol_deps(s.id, s.get_type(), deps);
  });
  ...
  context.foreach_operand([&worklist](const symbolt &s) {
    if (library_ids.find(s.id) == library_ids.end())
      worklist.push_back(s.id);
  });

  std::unordered_set<irep_idt, irep_id_hash> live;
  while (!worklist.empty())
  {
    const irep_idt name = worklist.front();
    worklist.pop_front();
    if (!live.insert(name).second)
      continue;
    auto range = deps.equal_range(name);
    for (auto it = range.first; it != range.second; ++it)
      worklist.push_back(it->second);
  }

  std::vector<irep_idt> doomed;
  context.foreach_operand([&doomed, &live](const symbolt &s) {
    if (
      !s.is_type && s.get_type().is_code() &&
      library_ids.find(s.id) != library_ids.end() &&
      live.find(s.id) == live.end())
      doomed.push_back(s.id);
  });

  for (const irep_idt &id : doomed)
  {
    pruned_ids.insert(id);
    context.erase_symbol(id);
  }

  log_debug(
    "c2goto",
    "library prune: {} of {} library symbols erased, {} kept live",
    doomed.size(),
    library_ids.size(),
    live.size());
}
...
void assert_no_pruned_calls(const goto_functionst &functions)
{
  ...
  for (const auto &named : functions.function_map)
    for (const auto &ins : named.second.body.instructions)
    {
      if (!ins.is_function_call())
        continue;
      const expr2tc &callee = to_code_function_call2t(ins.code).function;
      if (!is_symbol2t(callee))
        continue;
      const irep_idt &id = to_symbol2t(callee).thename;
      if (pruned_ids.find(id) == pruned_ids.end())
        continue;
      log_error(
        "library prune removed '{}', but {} calls it. The reference was "
        "introduced after the prune, so its dependency closure is incomplete.",
        id2string(id),
        id2string(named.first));
      abort();
    }
}
...
void add_cprover_library(contextt &context, const languaget *language)
{
  ...
  store_ctx.foreach_operand(
    [](const symbolt &s) { record_library_symbol(s.id); });
