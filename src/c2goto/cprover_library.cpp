...
/* This function uses the headers generated by the build system to dump the
 * bundled libc/libm files in a temporary directory, parse and link them. The
 * headers contain invocations of the form ESBMC_FLAIL(body, size, name) for
 * each bundled file, see scripts/flail.py --macro.
 *
  ...
  const languaget &c_language)
{
  ...
  tmp_path tmp = create_tmp_dir();

  /* replicate source tree directory structure in tmp */
  std::string library = tmp.path() + "/library";
  std::string libm = library + "/libm";
  std::string headers = tmp.path() + "/headers";
...
#define ESBMC_FLAIL(body, size, ...)                                           \
  process(body, size, library + "/" #__VA_ARGS__);
#include <libc.h>
#undef ESBMC_FLAIL

#define ESBMC_FLAIL(body, size, ...)                                           \
  process(body, size, libm + "/" #__VA_ARGS__);
#include <libm.h>
#undef ESBMC_FLAIL
}
