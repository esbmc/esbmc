...
public:
  static void *rvalue_cvt(const Container *type, Base *out)
  {
    ...
    std::cerr << "rvalue of irep2tc_to_irep2t should never be called"
              << std::endl;
    abort();
  }
...
public:
  ...
  static void *lvalue_cvt(const DownType *foo)
  {
    ...
    return const_cast<void *>(
      reinterpret_cast<const void *>(dynamic_cast<const BaseType *>(foo)));
  }
};
...
static std::vector<std::string> illegal_python_names = {"not",
                                                        "or",
                                                        "and",
                                                        "with"};
...
void build_base_type2t_python_class(void)
{
  using namespace boost::python;
  class_<type2t, boost::noncopyable, irep_container<type2t>> foo(
    "type2t", no_init);
  register_irep_methods<type2t> bar;
  bar(foo, "type");

  // Register our manual type2tc -> type2t converter.
  esbmc_python_cvt<type2t,
                   type2tc,
                   false,
                   true,
                   true,
                   irep2tc_to_irep2t<type2tc, type2t>>();
  // None converter
  esbmc_python_cvt<type2tc,
                   char,
                   true,
                   true,
                   false,
                   none_to_irep2tc<type2tc>>();

  enum_<type2t::type_ids>("type_ids")
#define hahatemporary(r, data, elem)                                           \
  .value(BOOST_PP_STRINGIZE(elem), type2t::BOOST_PP_CAT(elem, _id))
...
void build_type2t_container_converters(void)
{
// Needs to be called _after_ the type types are registered.
#define hahatemporary(r, data, elem)                                           \
  esbmc_python_cvt<BOOST_PP_CAT(elem, _type2t),                                \
                   BOOST_PP_CAT(elem, _type2tc),                               \
                   false,                                                      \
                   true,                                                       \
                   true,                                                       \
                   irep2tc_to_irep2t<BOOST_PP_CAT(elem, _type2tc),             \
...
#define hahatemporary(r, data, elem)                                           \
  esbmc_python_cvt<                                                            \
    ...
    true,                                                                      \
    true,                                                                      \
    irep2tc_to_irep2tc<BOOST_PP_CAT(elem, _type2tc), type2tc>>();
  BOOST_PP_LIST_FOR_EACH(hahatemporary, foo, ESBMC_LIST_OF_TYPES)
#undef hahatemporary
...
template <typename T>
void filter_illegal_python_names(
  T enumids,
  const char *str,
  expr2t::expr_ids id)
{
  ...
  if(
    std::find(
      illegal_python_names.begin(),
      illegal_python_names.end(),
      filtered_pyname) != illegal_python_names.end())
...
void build_base_expr2t_python_class(void)
{
  using namespace boost::python;
  class_<expr2t, boost::noncopyable, irep_container<expr2t>> foo(
    "expr2t", no_init);
  register_irep_methods<expr2t> bar;
  bar(foo, "expr2t");

  // Register our manual expr2tc -> expr2t converter.
  esbmc_python_cvt<expr2t,
                   expr2tc,
                   false,
                   true,
                   true,
                   irep2tc_to_irep2t<expr2tc, expr2t>>();
  // None converter
  esbmc_python_cvt<expr2tc,
                   char,
                   true,
                   true,
                   false,
                   none_to_irep2tc<expr2tc>>();

  {
    enum_<expr2t::expr_ids> enumids("expr_ids");
#define hahatemporary(r, data, elem)                                           \
  filter_illegal_python_names(                                                 \
    enumids, BOOST_PP_STRINGIZE(elem), expr2t::BOOST_PP_CAT(elem, _id));
...
void build_expr2t_container_converters(void)
{
// Needs to be called _after_ the expr types are registered.
#define hahatemporary(r, data, elem)                                           \
  esbmc_python_cvt<                                                            \
    BOOST_PP_CAT(elem, 2t),                                                    \
    BOOST_PP_CAT(elem, 2tc),                                                   \
    false,                                                                     \
    true,                                                                      \
    true,                                                                      \
    irep2tc_to_irep2t<BOOST_PP_CAT(elem, 2tc), BOOST_PP_CAT(elem, 2t)>>();
  BOOST_PP_LIST_FOR_EACH(hahatemporary, foo, ESBMC_LIST_OF_EXPRS)
#undef hahatemporary
...
#define hahatemporary(r, data, elem)                                           \
  esbmc_python_cvt<expr2tc,                                                    \
                   ...
                   true,                                                       \
                   true,                                                       \
                   irep2tc_to_irep2tc<BOOST_PP_CAT(elem, 2tc), expr2tc>>();
  BOOST_PP_LIST_FOR_EACH(hahatemporary, foo, ESBMC_LIST_OF_EXPRS)
#undef hahatemporary
  ...
  int indent __attribute__((unused)))
{
  return (data == sideeffect_data::allockind::malloc)
           ? "malloc"
           : (data == sideeffect_data::allockind::realloc)
               ? "realloc"
               : (data == sideeffect_data::allockind::alloca)
                   ? "alloca"
                   : (data == sideeffect_data::allockind::cpp_new)
                       ? "cpp_new"
                       : (data == sideeffect_data::allockind::cpp_new_arr)
                           ? "cpp_new_arr"
                           : (data == sideeffect_data::allockind::nondet)
                               ? "nondet"
                               : (data == sideeffect_data::allockind::va_arg)
                                   ? "va_arg"
                                   : (data ==
                                      sideeffect_data::allockind::function_call)
                                       ? "function_call"
                                       : "unknown";
}
...
template <>
void call_type_delegate<const std::vector<type2tc>,
...
void esbmct::
  irep_methods2<derived, baseclass, traits, container, enable, fields>::
    build_python_class(const typename container::id_field_type id)
{
#ifdef WITH_PYTHON
  using namespace boost::python;
  ...
  if(
    std::find(
      illegal_python_names.begin(), illegal_python_names.end(), basename_str) !=
    illegal_python_names.end())
    basename_str.append("_");

  class_<derived, bases<base2t>, container, boost::noncopyable> foo(
    basename_str.c_str(), no_init);
...
{
public:
  static bool value(R T::*foo __attribute__((unused)))
  {
    return false;
  }
};
...
template <>
class field_to_be_skipped<type2t::type_ids, type2t>
{
public:
  static bool value(type2t::type_ids type2t::*foo)
  {
    if(foo == &type2t::type_id)
      return true;
    else
      return false;
  }
...
template <>
class field_to_be_skipped<const expr2t::expr_ids, expr2t>
{
public:
  static bool value(const expr2t::expr_ids expr2t::*foo)
  {
    if(foo == &expr2t::expr_id)
      return true;
    else
      return false;
  }
...
template <>
class field_to_be_skipped<type2tc, expr2t>
{
public:
  static bool value(type2tc expr2t::*foo)
  {
    if(foo == &expr2t::type)
      return true;
    else
...
void esbmct::
  irep_methods2<derived, baseclass, traits, container, enable, fields>::
    build_python_class_rec(T &obj, unsigned int idx)
{
#ifdef WITH_PYTHON
...
std::string union_type2t::field_names[esbmct::num_type_fields] =
  {"members", "member_names", "member_pretty_names", "typename", "packed", ""};
std::string unsignedbv_type2t::field_names[esbmct::num_type_fields] = {"width",
                                                                       "",
                                                                       "",
                                                                       "",
                                                                       ""};
std::string signedbv_type2t::field_names[esbmct::num_type_fields] = {"width",
                                                                     "",
                                                                     "",
                                                                     "",
                                                                     ""};
...
std::string pointer_type2t::field_names[esbmct::num_type_fields] = {"subtype",
                                                                    "",
                                                                    "",
                                                                    "",
                                                                    ""};
std::string fixedbv_type2t::field_names[esbmct::num_type_fields] =
  {"width", "integer_bits", "", "", ""};
std::string floatbv_type2t::field_names[esbmct::num_type_fields] = {"fraction",
                                                                    "exponent",
                                                                    "",
                                                                    "",
                                                                    ""};
std::string string_type2t::field_names[esbmct::num_type_fields] = {"width",
                                                                   "",
                                                                   "",
                                                                   "",
                                                                   ""};
...
std::string constant_int2t::field_names[esbmct::num_type_fields] = {"value",
                                                                    "",
                                                                    "",
                                                                    "",
                                                                    ""};
std::string constant_fixedbv2t::field_names[esbmct::num_type_fields] = {"value",
                                                                        "",
                                                                        "",
                                                                        "",
                                                                        ""};
std::string constant_floatbv2t::field_names[esbmct::num_type_fields] = {"value",
                                                                        "",
                                                                        "",
                                                                        "",
                                                                        ""};
std::string constant_struct2t::field_names[esbmct::num_type_fields] =
  {"members", "", "", "", ""};
std::string constant_union2t::field_names[esbmct::num_type_fields] = {"members",
                                                                      "",
                                                                      "",
                                                                      "",
                                                                      ""};
std::string constant_bool2t::field_names[esbmct::num_type_fields] = {"value",
                                                                     "",
                                                                     "",
                                                                     "",
                                                                     ""};
std::string constant_array2t::field_names[esbmct::num_type_fields] = {"members",
                                                                      "",
                                                                      "",
                                                                      "",
                                                                      ""};
std::string constant_array_of2t::field_names[esbmct::num_type_fields] =
  {"initializer", "", "", "", ""};
std::string constant_string2t::field_names[esbmct::num_type_fields] = {"value",
                                                                       "",
                                                                       "",
                                                                       "",
                                                                       ""};
...
std::string nearbyint2t::field_names[esbmct::num_type_fields] =
  {"from", "rounding_mode", "", "", "", ""};
std::string if2t::field_names[esbmct::num_type_fields] = {"cond",
                                                          "true_value",
                                                          "false_value",
                                                          "",
                                                          ""};
std::string equality2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                                "side_2",
                                                                "",
                                                                "",
                                                                ""};
std::string notequal2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                                "side_2",
                                                                "",
                                                                "",
                                                                ""};
std::string lessthan2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                                "side_2",
                                                                "",
                                                                "",
                                                                ""};
std::string greaterthan2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                                   "side_2",
                                                                   "",
                                                                   "",
                                                                   ""};
std::string lessthanequal2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                                     "side_2",
                                                                     "",
                                                                     "",
                                                                     ""};
std::string greaterthanequal2t::field_names[esbmct::num_type_fields] =
  {"side_1", "side_2", "", "", ""};
std::string not2t::field_names[esbmct::num_type_fields] = {"value",
                                                           "",
                                                           "",
                                                           "",
                                                           ""};
std::string and2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                           "side_2",
                                                           "",
                                                           "",
                                                           ""};
std::string or2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                          "side_2",
                                                          "",
                                                          "",
                                                          ""};
std::string xor2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                           "side_2",
                                                           "",
                                                           "",
                                                           ""};
std::string implies2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                               "side_2",
                                                               "",
                                                               "",
                                                               ""};
std::string bitand2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                              "side_2",
                                                              "",
                                                              "",
                                                              ""};
std::string bitor2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                             "side_2",
                                                             "",
                                                             "",
                                                             ""};
std::string bitxor2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                              "side_2",
                                                              "",
                                                              "",
                                                              ""};
std::string bitnand2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                               "side_2",
                                                               "",
                                                               "",
                                                               ""};
std::string bitnor2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                              "side_2",
                                                              "",
                                                              "",
                                                              ""};
std::string bitnxor2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                               "side_2",
                                                               "",
                                                               "",
                                                               ""};
std::string lshr2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                            "side_2",
                                                            "",
                                                            "",
                                                            ""};
std::string bitnot2t::field_names[esbmct::num_type_fields] = {"value",
                                                              "",
                                                              "",
                                                              "",
                                                              ""};
std::string neg2t::field_names[esbmct::num_type_fields] = {"value",
                                                           "",
                                                           "",
                                                           "",
                                                           ""};
std::string abs2t::field_names[esbmct::num_type_fields] = {"value",
                                                           "",
                                                           "",
                                                           "",
                                                           ""};
std::string add2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                           "side_2",
                                                           "",
                                                           "",
                                                           ""};
std::string sub2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                           "side_2",
                                                           "",
                                                           "",
                                                           ""};
std::string mul2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                           "side_2",
                                                           "",
                                                           "",
                                                           ""};
std::string div2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                           "side_2",
                                                           "",
                                                           "",
                                                           ""};
...
std::string ieee_sqrt2t::field_names[esbmct::num_type_fields] =
  {"rounding_mode", "value", "", "", ""};
std::string modulus2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                               "side_2",
                                                               "",
                                                               "",
                                                               ""};
std::string shl2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                           "side_2",
                                                           "",
                                                           "",
                                                           ""};
std::string ashr2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                            "side_2",
                                                            "",
                                                            "",
                                                            ""};
std::string same_object2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                                   "side_2",
                                                                   "",
                                                                   "",
                                                                   ""};
...
std::string pointer_object2t::field_names[esbmct::num_type_fields] =
  {"pointer_obj", "", "", "", ""};
std::string address_of2t::field_names[esbmct::num_type_fields] = {"pointer_obj",
                                                                  "",
                                                                  "",
                                                                  "",
                                                                  ""};
...
std::string with2t::field_names[esbmct::num_type_fields] = {"source_value",
                                                            "update_field",
                                                            "update_value",
                                                            "",
                                                            ""};
std::string member2t::field_names[esbmct::num_type_fields] = {"source_value",
                                                              "member_name",
                                                              "",
                                                              "",
                                                              ""};
std::string index2t::field_names[esbmct::num_type_fields] = {"source_value",
                                                             "index",
                                                             "",
                                                             "",
                                                             ""};
std::string isnan2t::field_names[esbmct::num_type_fields] = {"value",
                                                             "",
                                                             "",
                                                             "",
                                                             ""};
std::string overflow2t::field_names[esbmct::num_type_fields] = {"operand",
                                                                "",
                                                                "",
                                                                "",
                                                                ""};
std::string overflow_cast2t::field_names[esbmct::num_type_fields] = {"operand",
                                                                     "bits",
                                                                     "",
                                                                     "",
                                                                     ""};
std::string overflow_neg2t::field_names[esbmct::num_type_fields] = {"operand",
                                                                    "",
                                                                    "",
                                                                    "",
                                                                    ""};
...
std::string dereference2t::field_names[esbmct::num_type_fields] = {"pointer",
                                                                   "",
                                                                   "",
                                                                   "",
                                                                   ""};
std::string valid_object2t::field_names[esbmct::num_type_fields] = {"value",
                                                                    "",
                                                                    "",
                                                                    "",
                                                                    ""};
std::string deallocated_obj2t::field_names[esbmct::num_type_fields] = {"value",
                                                                       "",
                                                                       "",
                                                                       "",
                                                                       ""};
std::string dynamic_size2t::field_names[esbmct::num_type_fields] = {"value",
                                                                    "",
                                                                    "",
                                                                    "",
                                                                    ""};
std::string sideeffect2t::field_names[esbmct::num_type_fields] = {"operand",
                                                                  "size",
                                                                  "arguments",
                                                                  "alloctype",
                                                                  "kind"};
std::string code_block2t::field_names[esbmct::num_type_fields] = {"operands",
                                                                  "",
                                                                  "",
                                                                  "",
                                                                  ""};
std::string code_assign2t::field_names[esbmct::num_type_fields] = {"target",
                                                                   "source",
                                                                   "",
                                                                   "",
                                                                   ""};
std::string code_init2t::field_names[esbmct::num_type_fields] = {"target",
                                                                 "source",
                                                                 "",
                                                                 "",
                                                                 ""};
std::string code_decl2t::field_names[esbmct::num_type_fields] = {"value",
                                                                 "",
                                                                 "",
                                                                 "",
                                                                 ""};
...
std::string code_expression2t::field_names[esbmct::num_type_fields] =
  {"operand", "", "", "", ""};
std::string code_return2t::field_names[esbmct::num_type_fields] = {"operand",
                                                                   "",
                                                                   "",
                                                                   "",
                                                                   ""};
std::string code_skip2t::field_names[esbmct::num_type_fields] = {"",
                                                                 ...
                                                                 "",
                                                                 ""};
std::string code_free2t::field_names[esbmct::num_type_fields] = {"operand",
                                                                 "",
                                                                 "",
                                                                 "",
                                                                 ""};
std::string code_goto2t::field_names[esbmct::num_type_fields] = {"target",
                                                                 "",
                                                                 "",
                                                                 "",
                                                                 ""};
...
std::string code_comma2t::field_names[esbmct::num_type_fields] = {"side_1",
                                                                  "side_2",
                                                                  "",
                                                                  "",
                                                                  ""};
std::string invalid_pointer2t::field_names[esbmct::num_type_fields] =
  {"pointer_obj", "", "", "", ""};
std::string code_asm2t::field_names[esbmct::num_type_fields] = {"value",
                                                                "",
                                                                "",
                                                                "",
                                                                ""};
std::string code_cpp_del_array2t::field_names[esbmct::num_type_fields] =
  {"value", "", "", "", ""};
std::string code_cpp_delete2t::field_names[esbmct::num_type_fields] = {"value",
                                                                       "",
                                                                       "",
                                                                       "",
                                                                       ""};
...
std::string code_cpp_throw_decl_end2t::field_names[esbmct::num_type_fields] =
  {"exception_list", "", "", "", ""};
std::string isinf2t::field_names[esbmct::num_type_fields] = {"value",
                                                             "",
                                                             "",
                                                             "",
                                                             ""};
std::string isnormal2t::field_names[esbmct::num_type_fields] = {"value",
                                                                "",
                                                                "",
                                                                "",
                                                                ""};
std::string isfinite2t::field_names[esbmct::num_type_fields] = {"value",
                                                                "",
                                                                "",
                                                                "",
                                                                ""};
std::string signbit2t::field_names[esbmct::num_type_fields] = {"value",
                                                               "",
                                                               "",
                                                               "",
                                                               ""};
std::string concat2t::field_names[esbmct::num_type_fields] = {"forward",
                                                              ...
                                                              "",
                                                              ""};
std::string extract2t::field_names[esbmct::num_type_fields] = {"from",
                                                               "upper",
                                                               "lower",
                                                               "",
                                                               ""};
...
#define irep_typedefs0(basename, superclass)                                   \
  template class esbmct::irep_methods2<basename##2t,                           \
                                       superclass,                             \
                                       superclass::traits,                     \
                                       basename##2tc>;                         \
...
#define type_typedefs1(basename, superclass)                                   \
  template class esbmct::type_methods2<basename##2t,                           \
                                       superclass,                             \
...
#define expr_typedefs1(basename, superclass)                                   \
  template class esbmct::expr_methods2<basename##2t,                           \
                                       superclass,                             \
                                       superclass::traits,                     \
                                       basename##2tc>;                         \
  irep_typedefs1(basename, superclass)
