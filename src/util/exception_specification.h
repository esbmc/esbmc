...
public:
  ...
  /// True if an exception of the given (already-expanded) type id may escape
  /// the function without violating the specification.
  bool allows(const irep_idt &exception_type_id) const;
  ...
  /// Write this specification onto a function typet as named-sub metadata.
  void to_type(typet &type) const;
