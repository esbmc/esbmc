...
void base_type(typet &type, const namespacet &ns)
{
  ...
  else if(type.id() == "struct" || type.id() == "class" || type.id() == "union")
  {
...
bool base_type_eqt::base_type_eq_rec(const typet &type1, const typet &type2)
{
  ...
  {
    // already in same set?
    if(identifiers.make_union(type1.identifier(), type2.identifier()))
      return true;
  }
  ...
  if(type1.id() == "struct" || type1.id() == "class" || type1.id() == "union")
  {
...
bool base_type_eqt::base_type_eq_rec(const expr2tc &expr1, const expr2tc &expr2)
{
...
static std::string reformat_class_name(const std::string &from)
{
