...
void irep_serializationt::reference_convert(std::istream &in, irept &irep)
{
  ...
  {
    read_irep_helper(in, irep);
    ireps_container.ireps_on_read[id] = irep;
  }
}

void irep_serializationt::read_irep_helper(std::istream &in, irept &irep)
{
...
dstring irep_serializationt::read_string(std::istream &in)
{
  ...
  read_buffer.resize(
    1, 0); // Initializes read_buffer with 1 element containing 0
...
template <>
void irep_serializationt::write_irep2(std::ostream &out, const type2tc &irep)
{
  irep_serializationt::ireps_containert ic;
  ic.clear();
  irep_serializationt obj(ic);
  const irept &t = migrate_type_back(irep);
  obj.write_irep(out, t);
}

template <>
void irep_serializationt::write_irep2(std::ostream &out, const expr2tc &irep)
{
  irep_serializationt::ireps_containert ic;
  ic.clear();
  irep_serializationt obj(ic);
