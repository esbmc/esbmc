...
expr2t::simplify() const
{
  ...
  catch (array_type2t::dyn_sized_array_excp *e)
  {
    ...
    // don't even attempt partial simplification. We'd probably have to double
...
modulus2t::do_simplify() const
{
  ...
  if(is_bv_type(type))
  {
    ...
    if (is_constant_int2t(side_1) && is_constant_int2t(side_2))
    {
      ...
      return constant_int2tc(type, n.value % d.value);
    }
  }
...
with2t::do_simplify() const
{
  ...
  if (is_constant_array2t(source_value) && is_constant_int2t(update_field))
  {
...
expr2tc
pointer_offset2t::do_simplify() const
{
  ...
  if (is_add2t(ptr_obj))
  {
    ...
    expr2tc non_ptr_op = is_pointer_type(add.side_1) ? add.side_2 : add.side_1;
...
typecast2t::do_simplify() const
{
  ...
  if (is_constant_expr(simp))
  {
    ...
    if (is_bv_type(simp) && is_number_type(type))
    {
    ...
    if (is_fixedbv_type(simp) && is_number_type(type))
    {
    ...
    if (is_floatbv_type(simp) && is_number_type(type))
    {
      // float/double to int/float/double
      if(!is_constant_int2t(rounding_mode))
        return expr2tc();
  ...
  if (is_pointer_type(type) && is_pointer_type(simp))
  {
    ...
    if (is_symbol_type(ptr_to.subtype)
        || is_symbol_type(ptr_from.subtype)
        || is_code_type(ptr_to.subtype)
        || is_code_type(ptr_from.subtype))
  ...
  if (is_typecast2t(simp) && type == simp->type)
...
expr2tc
address_of2t::do_simplify() const
{
...
expr2tc
greaterthan2t::do_simplify() const
{
...
expr2tc
lessthanequal2t::do_simplify() const
{
...
expr2tc
overflow_cast2t::do_simplify() const
{
  return expr2tc();
}

expr2tc
overflow2t::do_simplify() const
{
  return expr2tc();
}
...
obj_equals_addr_of(const expr2tc &a, const expr2tc &b)
{
  ...
  if (is_symbol2t(a) && is_symbol2t(b) && (a == b))
      return gen_true_expr();

  if (is_index2t(a) && is_index2t(b))
    return obj_equals_addr_of(to_index2t(a).source_value, to_index2t(b).source_value);

  if (is_member2t(a) && is_member2t(b))
    return obj_equals_addr_of(to_member2t(a).source_value, to_member2t(b).source_value);

  if (is_constant_string2t(a) && is_constant_string2t(b))
  {
...
expr2tc
same_object2t::do_simplify() const
{
