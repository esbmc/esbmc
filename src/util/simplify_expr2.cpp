...
template <typename constant,
          ...
          typename div_type>
static expr2tc
simplify_multiplication(const expr2tc &op1,
       const expr2tc &op2,
       const div_type &type,
       std::function<bool(const expr2tc&)> is_constant,
       std::function<constant_value_type(const expr2tc&)> get_value)
{
  if(is_constant(op1))
  {
    ...
    if(get_value(op1).is_zero())
      return expr2tc(new constant(type, constant_value_type()));
...
mul2t::do_simplify(bool second __attribute__((unused))) const
{
  ...
  if(is_bv_type(type))
  {
    ...
    return simplify_multiplication<constant_int2t, BigInt, decltype(type)>(
  ...
  else if(is_fixedbv_type(type))
  {
    ...
    return simplify_multiplication<constant_fixedbv2t, fixedbvt, decltype(type)>(
      to_simplify_side_1, to_simplify_side_2, type, is_constant, get_value);
  }
  else if(is_floatbv_type(type))
  {
    ...
    return simplify_multiplication<constant_floatbv2t, ieee_floatt, decltype(type)>(
      to_simplify_side_1, to_simplify_side_2, type, is_constant, get_value);
  }
