...
typecast2t::do_simplify(bool second) const
{
  ...
  if (is_constant_expr(new_from))
  {
    ...
    if (is_bool_type(new_from) && is_number_type(type))
    {
      ...
      {
        // bool to int
        return constant_int2tc(type, BigInt(to_constant_bool2t(new_from).value));
      }
  ...
  else if (is_bool_type(type))
  {
    ...
    return notequal2tc(new_from, zero2);
  }
  ...
  else if (is_typecast2t(new_from) && type == new_from->type)
  {
    ...
    return typecast2tc(type, to_typecast2t(new_from).from);
  }
  else if (second
           && is_bv_type(type)
           && is_bv_type(new_from)
