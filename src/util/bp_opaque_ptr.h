...
// XXX jmorse: Boost.Python trips up over const classes being fed into their
// opaque pointer implementation. This is significant because b.p distinguishes
// between const and non-const in it's type_id implementation. All the
// infrastructure in opaque<> is private too, so I can't just manually insert
// conversion ptrs. We could register non-const then hack it; but that's
// highly brittle. Instead, vendor their code, which is entirely unjustifiable
// up to the point where it's necessary.

// Taken from libboost-python 1.55.0 on debian jessie.

# ifndef _ESBMC_OPAQUE_POINTER_CONVERTER_HPP_
# define _ESBMC_OPAQUE_POINTER_CONVERTER_HPP_
...
struct opaque_const
{
    ...
    static opaque_const instance;
private:
    
    static void* extract(PyObject* op)
    {
      // XXX jmorse: this is where const hackery is required.
        if (PyObject_TypeCheck(op, &type_object))
            return const_cast<typename std::remove_const<Pointee>::type*>(static_cast<python_instance*>(implicit_cast<void*>(op))->x);
        else
            return NULL;
    }
...
template <class Pointee>
opaque_const<Pointee> opaque_const<Pointee>::instance;

template <class Pointee>
PyTypeObject opaque_const<Pointee>::type_object =
{
    PyVarObject_HEAD_INIT(NULL, 0)
    0,
    sizeof( BOOST_DEDUCED_TYPENAME opaque_const<Pointee>::python_instance ),
    0,
...
# endif    // _ESBMC_OPAQUE_POINTER_CONVERTER_HPP_
