...
void cmdlinet::parse(int argc, const char **argv)
{
  ...
  group0.add_options()("input-file",boost::program_options::value<std::vector<std::string>>()->value_name("file.c ..."),"source file names");
  ...
  ("smt-model","show SMT model (not supported by all solvers), if the formula is SAT");
  ...
  ("witness-output", boost::program_options::value<std::string>(), "generate the verification result witness in GraphML format")
  ...
  group5.add_options()
  ("function", boost::program_options::value<std::string>()->value_name("name"), "set main function name")
  ("claim", boost::program_options::value<int>()->value_name("nr"), "only check specific claim")
  ("depth", boost::program_options::value<int>()->value_name("nr"), "limit search depth")
  ("unwind", boost::program_options::value<int>()->value_name("nr"), "unwind nr times")
  ("unwindset", boost::program_options::value<std::string>()->value_name("nr"), "unwind given loop nr times")
  ...
  ("k-step", boost::program_options::value<int>()->value_name("nr"), "set k increment (default is 1)");
  ...
  ("smtlib-solver-prog",boost::program_options::value<std::string>(),"SMT lib program name")
  ("output", boost::program_options::value<std::string>()->value_name("<filename>"), "output VCCs in SMT lib format to given file")
  ("floatbv", "encode floating-point using the SMT floating-point theory(default)")
  ("fixedbv", "encode floating-point as fixed bit-vectors")
  ("fp2bv", "encode floating-point as bit-vectors(default for solvers that don't ""support the SMT floating-point theory)")
  ...
  ("no-return-value-opt", "disable return value optimization to compute the stack size");
