...
void qdimacs_coret::simplify_extractbits(exprt &expr) const
{
  if(expr.id()=="and")
  {
    ...
    {
      if(it->id()=="extractbit" && it->op1().is_constant())
      {
        used_bits_map[it->op0()].insert(it->op1());
      }
      else if(it->id()=="not" &&
              it->op0().id()=="extractbit" && it->op0().op1().is_constant())
      {
        ...
        it++)
    {
      ...
      {
        const irep_idt &ident=it->first.get("identifier");
        ...
        for(exprt::operandst::const_iterator oit=old_operands.begin();
            ...
            oit++)
        {
          if(oit->id()=="extractbit" &&
             oit->op1().is_constant())
          {
            if(oit->op0().get("identifier")==ident)
            {
              ...
              continue;
                  ...
                  oit->op0().id()=="extractbit" &&
                  oit->op0().op1().is_constant())
          {
            if(oit->op0().op0().get("identifier")==ident)
            {
        ...
        exprt new_value("constant", it->first.type());
        new_value.set("value", value_string);
