...
z3_convt::convert_smt_type(const unsignedbv_type2t &type, void *&_bv)
{
  ...
  if (int_encoding) {
    bv = Z3_mk_int_sort(z3_ctx);
  } else {
    unsigned int width = type.get_width();
    bv = Z3_mk_bv_sort(z3_ctx, width);
  }
...
z3_convt::convert_smt_type(const array_type2t &type, void *&_bv)
{
  ...
  if (int_encoding) {
    idx_sort = Z3_mk_int_sort(z3_ctx);
  } else {
    idx_sort = Z3_mk_bv_sort(z3_ctx, config.ansi_c.int_width);
  }
                                    ...
                                    void *&_bv)
{
  ...
  if (uni) {
    ...
    if (int_encoding)
      proj_types[num_elems - 1] = Z3_mk_int_sort(z3_ctx);
    else
      proj_types[num_elems - 1] = Z3_mk_bv_sort(z3_ctx,
...
z3_convt::convert_smt_expr(const constant_int2t &sym, void *&_bv)
{
  ...
  if (int_encoding)
    ...
    int_sort = Z3_mk_bv_sort(z3_ctx, bitwidth);
...
z3_convt::convert_smt_expr(const constant_fixedbv2t &sym, void *&_bv)
{
  ...
  if (int_encoding) {
    ...
    magnitude =
      Z3_mk_int(z3_ctx, atoi(m.c_str()), Z3_mk_bv_sort(z3_ctx, bitwidth / 2));
    fraction =
      Z3_mk_int(z3_ctx, atoi(f.c_str()), Z3_mk_bv_sort(z3_ctx, bitwidth / 2));
...
z3_convt::convert_typecast_bool(const typecast2t &cast, Z3_ast &bv)
{
  ...
  if (is_bv_type(cast.from->type) ||
    ...
    if (int_encoding)
      ...
      args[1] =
        Z3_mk_int(z3_ctx, 0, Z3_mk_bv_sort(z3_ctx, config.ansi_c.int_width));
...
z3_convt::convert_typecast_to_ints(const typecast2t &cast, Z3_ast &bv)
{
  ...
  if (is_signedbv_type(cast.from->type) ||
    ...
    if (is_signedbv_type(cast.type)) {
      if (int_encoding) {
	zero = Z3_mk_int(z3_ctx, 0, Z3_mk_int_sort(z3_ctx));
	one = Z3_mk_int(z3_ctx, 1, Z3_mk_int_sort(z3_ctx));
      } else   {
