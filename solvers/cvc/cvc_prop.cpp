...
Revision: Lucas Cordeiro, lcc08r@ecs.soton.ac.uk
...
\*******************************************************************/
#if 0
void cvc_propt::l_set_to(literalt a, bool value)
{
  bvt bv;
...
Function: cvc_propt::new_literal
...
#if 0
literalt cvc_propt::new_literal()
{
  _no_literals++;
  out << "l" << _no_literals << ": BOOLEAN;" << std::endl;
  literalt l;
  l.set(_no_literals, false);
  return l;
}
#endif
/*******************************************************************\
...
void cvc_propt::lcnf(const bvt &bv)
{
  ...
  {
	  bvt::const_iterator it=new_bv.begin();
	  out << cvc_literal(*it);
  }
