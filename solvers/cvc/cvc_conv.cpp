...
Author: Daniel Kroening, kroening@kroening.com
	    Lucas Cordeiro, lucascordeiro@ecs.soton.ac.uk
...
literalt cvc_convt::convert_rest(const exprt &expr)
{
	...
	//lucas
	  if(expr.id()=="<")
	  {
	    ...
	    cvc_prop.out << "ASSERT " << cvc_prop.cvc_literal(l) << " <=> BVLT(";
	  ...
	  {
	    assert(expr.operands().size()==2);
	    cvc_prop.out << "ASSERT " << cvc_prop.cvc_literal(l) << " <=> BVGT(";
	    convert_cvc_expr(expr.op0());
	    cvc_prop.out << ",";
	    convert_cvc_expr(expr.op1());
	    cvc_prop.out << ");" << std::endl;
	  ...
	  {
	    assert(expr.operands().size()==2);
	    cvc_prop.out << "ASSERT " << cvc_prop.cvc_literal(l) << " <=> BVLE(";
	    convert_cvc_expr(expr.op0());
	    cvc_prop.out << ",";
	    convert_cvc_expr(expr.op1());
	    cvc_prop.out << ");" << std::endl;
	  ...
	  {
	    assert(expr.operands().size()==2);
	    cvc_prop.out << "ASSERT " << cvc_prop.cvc_literal(l) << " <=> BVGE(";
	    convert_cvc_expr(expr.op0());
	    cvc_prop.out << ",";
	    convert_cvc_expr(expr.op1());
	    cvc_prop.out << ");" << std::endl;
...
void cvc_convt::convert_cvc_expr(const exprt &expr)
{
	  ...
	  {
	    convert_identifier("nondet_"+expr.get_string("identifier"));
	  }
	  else if(expr.id()=="typecast")
	  {
	            ...
	            expr.type().id()=="fixedbv")
	    {
	      ...
	      else if (op.type().id_string()=="c_enum")
	      {
	    	  convert_cvc_expr(op);
	      }
	      else
	    ...
	    else if (expr.type().id_string()=="c_enum")
	    {
	  	  convert_cvc_expr(op);
	    }
	    else
	  ...
	  else if(expr.id()=="constant")
	  {
	       ...
	       expr.type().id()=="bv")
	    {
	      if(cvc_bin2int)
	      {
	    	cvc_prop.out << binary2integer(expr.get("value").c_str(), false);
	    	cvc_bin2int=false;
	      }
	      else
	      {
	    	  //if (!(expr.get("value").operator !=("11111111111111111111111111111111")))
	    	    //cvc_prop.out << bin_zero(config.ansi_c.pointer_width);
	    	  //else
	    	std::string value_str;
	    	int value_int;
	    	value_str = integer2string(binary2integer(expr.get("value").c_str(), false), 10);
	    	value_int = atoi(value_str.c_str());

	      	if (value_int>=0)
	      	  cvc_prop.out << "0bin" << expr.get("value");
	      	else
	      	  cvc_prop.out << "BVUMINUS(" << "0bin" << expr.get("value") << ")";
	    ...
	    else if(expr.type().id()=="pointer")
	    {
	      ...
	      if(value=="NULL")
	      {
	        ...
	        throw "unknown pointer  "+id2string(value);
	    }
	    ...
	    {
	    	cvc_prop.out << "0bin"+integer2binary(binary2integer(expr.get("value").c_str(), false), config.ansi_c.int_width);
	    ...
	    else
	    {
	      ...
	      throw "unknown  "+expr.type().id_string();
	    }
	  }
	...
	//lucas
	          ...
	          expr.id()==">")
	  {
	    ...
	    if(op_type.id()=="unsignedbv" || op_type.id()=="signedbv")
	    {
	      if(expr.id()=="<=")
	        cvc_prop.out << "BVLE";
	      else if(expr.id()=="<")
	  ...
	  else if(expr.id()=="/")
	  {
	    ...
	    else if (expr.type().id()=="fixedbv")
	    {
	    	...
	    	mp_integer res;
	        int size;
	        res = binary2integer("0bin1", false)/binary2integer(expr.op1().get("value").c_str(), false);
	        size = atoi(expr.type().get("width").c_str());
	        cvc_prop.out << "0bin" << integer2binary(res,size);
	        */
	        cvc_prop.out << "(";
	        convert_cvc_expr(expr.op0());
	        cvc_prop.out << " * ";
