...
All 67 tests share the same configuration:
...
38 + 29 = 67, matching `ctest -L cxl` with `-DENABLE_CXL_REGRESSION=On`.
...
1. **Roadmap statistics** are now 38 passing / 29 bug-detecting across 67 suites. Note the figure that matters more: all 113 modelled functions are now exercised by some test, up from 22 of 105 (`scripts/cxl_model_coverage.py`). Test count is not coverage.
