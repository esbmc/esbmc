...
All 63 tests share the same configuration:
...
36 + 27 = 63, matching `ctest -L cxl` with `-DENABLE_CXL_REGRESSION=On`.
...
1. **Roadmap statistics** are now 36 passing / 27 bug-detecting across 63 suites. Note the figure that matters more: all 107 modelled functions are now exercised by some test, up from 22 of 105 (`scripts/cxl_model_coverage.py`). Test count is not coverage.
