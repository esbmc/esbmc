...
All 51 tests share the same configuration:
...
| **Additional flags** | `--memory-leak-check --overflow-check --unsigned-overflow-check`; the five concurrency tests use `--data-races-check --context-bound 2 --cswitch-skip-readonly-globals`; six tests add an explicit `--unwind` (see below) |
...
Unwinding is left to BMC's default where the loops are concretely bounded (`cxl_driver_irq_01`'s runs 10 iterations, `cxl_device_init_01` and the `cxl_mmio_readback_*` pair likewise). Six tests must state an `--unwind` instead, and the reason is worth knowing: a loop whose bound is a *mutable counter* in the model — `esbmc_irq_count`, or a device-supplied register value — stops being concrete as soon as a failure path merges, and symex then unwinds it indefinitely rather than solving the guard. `cxl_pci_config_01/02` use `--unwind 4`, `cxl_irq_msi_01/02` use `--unwind 2`, and `cxl_port_walk_01/02` use `--unwind 3`. Unwinding assertions are left **on** in all six, so the bound is proved rather than assumed.
...
30 + 21 = 51, matching `ctest -L cxl` with `-DENABLE_CXL_REGRESSION=On`.
...
1. **Roadmap statistics** are now 30 passing / 21 bug-detecting across 51 suites. Note the figure that matters more: 63 of 107 modelled functions are exercised by any test, up from 22 of 105 (`scripts/cxl_model_coverage.py`). Test count is not coverage.
