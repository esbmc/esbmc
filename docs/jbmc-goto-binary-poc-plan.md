...
What remains of Phase 1 is the measurement: re-run
`scripts/jbmc-poc-corpus/run-corpus.sh` and record the first unsupported
construct per program. That is now meaningful for the first time — before this
change every program died at the same SIGABRT with no attribution.
