...
This program is a real-life example designed to highlight the efficiency of GCSE optimization.
By enabling the `--gcse` switch in ESBMC, the C program can be verified significantly faster.

In this example, verification without GCSE fails due to an out-of-memory error on an Intel Xeon Platinum 8375C 32-core CPU with 128 GB of RAM.
