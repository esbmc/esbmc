...
3. **`numpy.searchsorted()`'s remaining gaps** — a genuine 2-D array input
   ...
   (`sorter=np.argsort(a)` over one is closed — see above).
4. **`.size` on an `eye`/`full`/`identity`/`linspace`-backed array** crashes
...
2. **`numpy.searchsorted()`'s remaining gaps** (~1 PR) — a genuine 2-D array
   ...
   array over a descriptor-resolved array (scalar/vector local-array-return
   resolution and `sorter=np.argsort(a)` over one are now implemented).
3. **`.size` fix** (~1 PR) — the `eye`/`full`-family `.size` crash,
