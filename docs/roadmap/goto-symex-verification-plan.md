...
so the check is defeated **iff `0 < k <= access_sz`** — the window is exactly one
access wide, and it is the `p[-1]` window.
...
form computed for it as well. Arrays are unaffected: they never reached this
function, which is why `int a[4]; *(int *)((char *)a - 4)` was already caught
and the struct spelling was not.
...
| `regression/esbmc/deref_negative_offset` | default | `SUCCESSFUL` on **both** binaries — an over-fire guard, not a discriminating test: every in-bounds byte offset still verifies, including the tight last one at `data_sz - access_sz` |
