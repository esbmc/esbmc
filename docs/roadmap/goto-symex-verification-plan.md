...
| **R50** | **Medium (no verdict, default configuration; a wrong answer and a solver abort behind a correct guard)** — found beside R49, §15 M9 (R49, R50); **FIXED**, §15 M9 (R50) | **A bound stored in a multi-dimensional array never folds, because propagating the array would break the encoder.** `int a[2][2]; a[1][1]=4;` looped to `a[1][1]` hangs; so does the 3-D spelling. `array_may_propagate` declined any multi-dimensional array that is not wholly constant. Ablating that clause makes the bound fold **and** breaks the encoder two ways: a read of the updated row at a nondet index aborts with `bitwuzla: error: ... expected array term at index 0`, and — worse — a row carrying **two** stores silently loses the older one, because `decompose_store_chain` walks only the newest update's spine. The gate's own comment named both modes; R50's row recorded only the aborts, because only the aborts had been measured. That second failure is a wrong answer, not a crash: `a[1][0]=5; a[1][1]=4;` read back through a row pointer returns `FAILED` on a correct program. | the three-configuration mutation matrix, §15 M9 (R50); `array_may_propagate` at `goto_symex_state.cpp:148`; `decompose_store_chain` at `smt_solver.cpp:2724`; `regression/esbmc/nested_array_{loop_bound,3d_bound,row_alias,row_ite,row_via_pointer}{,_fail}` | **H-C2** | Fixed in the encoder first, exactly as this row's earlier recommendation required, and only then in the gate. `lower_flattened_row_select` gives a read out of a row an encoding by pushing it inside the `with`/`ite` (select-over-store); `decompose_stores` normalises a whole chain — including every store a row carries — into flat element stores. With both in place the **dimension** restriction is unnecessary: the gate reverts to R42's own framing, bounded by element count alone, which fixes 3-D as well. |
...
Registered as R35 and pinned by
`regression/esbmc/deref_negative_offset_fail`; **fixed in §15 M9 (R35)**, where
the design question resolves to "out-of-bounds access" and the wrap that hid it
is spelt out. Note that `github_7127_fail` does **not** witness R35:
`member - 8` resolves to offset **0**, so its passing out-of-bounds claim is
correct.

**R35 was host-ABI-dependent, which is why the pin was scoped before the fix.**
`to_expr` materialises the offset as `gen_ulong(offset.to_int64())`, which
binds the `unsigned long` overload — 64 bits on LP64, **32 on LLP64**. So the
same descriptor reached `check_data_obj_access` as `2^64-4` on Linux/macOS and
as `2^32-4` on Windows. Only the first carried `offset + access_sz` back around
to `0`; `2^32-4 + 4` stays far above a 16-byte `data_sz`, so **Windows reported
the out-of-bounds and every other leg missed it**, and the KNOWNBUG pin carried
`REQUIRES lp64_host`. The fix retires the scoping: the rearranged check never
adds, so both widths are compared against the same bound and every host reports
the access.
...
confirms the two spellings collapse to one descriptor and moves the question
downstream of `to_expr`: a constant offset this far out of range is neither
performed nor flagged. A symbolic offset constrained to `-4` behaves
identically. §15 M9 (R35) answers it — the sum wraps back into range, and
constant folding then discharges the claim.
...
### M9 (R35) — 2026-08-21, the bound that the wrap carried back into range

R35 is fixed, and the mechanism is narrower and worse than "the lower bound is
not checked": it *is* checked, by an argument that holds everywhere except the
one window that matters.

**What the code said.** `dereferencet::check_data_obj_access` encodes
`offset + access_sz > data_sz` and explains that "lower bound [is] not checked,
instead we just treat everything as unsigned, which has the same effect". Every
premise of that is true. `bitsize_type2()` really is unsigned
(`get_uint_type(address_width + 3)`), and an offset below the object really does
arrive as a huge value — the unsimplified VCC for `*(int *)((char *)&s - 4)` is

```
(unsigned _ExtInt(67))0xFFFFFFFFFFFFFFFC * 8 + 32 > 128
```

**What it does.** That left operand is `2^67 - 32`, and adding the 32-bit access
size carries it to exactly `2^67`, which is `0` in the type. `0 > 128` is false,
so the claim is discharged — by constant folding, before the solver ever sees
it (`Generated 5 VCC(s), 0 remaining after simplification`). The rule
generalises: for an offset of `-k` bits the sum is `access_sz - k (mod 2^67)`,
so the check is defeated **iff `0 < k <= access_sz`** (given
`access_sz <= data_sz`; an over-sized access narrows the window to
`k < access_sz - data_sz`) — the window is exactly one access wide, and it is
the `p[-1]` window.

**Confirmed as a prediction, not a post-hoc story.** The rule says the hole
scales with the access, so the same program with a different access type must
move the boundary. Measured, all four as predicted:

| Access | `base - 1` | `base - 2` | `base - 4` | `base - 8` | `base - 16` |
|---|---|---|---|---|---|
| `char` | **SUCCESSFUL** | FAILED | — | — | — |
| `int` | — | — | **SUCCESSFUL** | FAILED | FAILED |
| `long` | — | — | — | **SUCCESSFUL** | FAILED |

**The fix keeps the unsigned reading and drops the addition.**
`offset + access_sz > data_sz` becomes `offset > data_sz - access_sz`, which is
the same predicate over the integers, evaluates its right-hand side in `BigInt`
where both operands are compile-time constants, and has nothing left to
overflow. The `access_sz > data_sz` case — an over-sized access, where the
subtraction would go negative — is the constant `true`, which is what the old
form computed for it *outside* the wrap window; inside it the new arm is
strictly stronger, as everywhere else. Arrays are unaffected: they never
reached this function, which is why `int a[4]; *(int *)((char *)a - 4)` was
already caught and the struct spelling was not.

**Two things the fix deliberately does not change.** The store below the base is
still not *performed* — R35's title says "neither performed nor flagged", and
only the flagging is a defect. The access is UB, so leaving its effect
unconstrained is the defensible reading, and under the default configuration the
violation fires before anything observes it; but with `--no-bounds-check` or
`--no-pointer-check` the dereference still drops the store silently. And the
sibling `bounds_check` on the array path keeps the wrapping form: its
`offset > arrsize` disjunct already covers the window, so it is correct, and the
rearrangement **must not** be copied there — `arrsize` can be a symbolic
`dynamic_size2tc`, where `arrsize - access_sz` underflows in the type and would
produce the mirror-image missed bug. What makes it safe here is that `data_sz`
and `access_sz` are compile-time `BigInt`s.

**Why this sat behind R34.** Reaching the window at all requires a *correct*
negative offset. Before #7127, `void *` subtraction moved forwards, so the
`void *` spellings of these programs corrupted a neighbour instead of arriving
below the base. Fixing the value set is what routed them here.

| Artefact | Invocation | Verdict |
|---|---|---|
| `regression/esbmc/deref_negative_offset_fail` | default | `FAILED` on `Access to object out of bounds`; `SUCCESSFUL` on the pre-patch control — the missed-bug direction |
...
R28's row proposed a diagnostic and nobody wrote it, so the silent case was
still silent. It is worse than the row implies. Four lines:
...
`--unwind 3 --no-unwinding-assertions` prints `Generated 0 VCC(s), 0 remaining`
and `VERIFICATION SUCCESSFUL`. Not one property was checked, the claim it
"proved" is false, and the output carries no warning — the VCC line is the only
trace and it reads as a simplification win. The same run with the assertions on
reports FAILED.

**The engine already knew.** `symex_goto.cpp:517` carries the comment "Nothing
will flag this truncation to the user" and calls `note_bounded_loop_truncation()`
beside it; `symex_resultt::bounded_loop_truncations` carries the count out. It
was read in exactly one place — `multi_property_check`, gated on
`--coverage-measurement` (#6387) — so an ordinary run threw it away. The fix is
to keep the flag on `bmct` and let `report_success()` spend it, which is the one
place a user learns the run proved anything.

**Wording is an interface decision.** `parse_result()` in `esbmc-wrapper.py`
classifies by substring over ESBMC's whole output, so the text avoids
`unwinding assertion loop`, `Timed out`, `Out of memory` and both verdict
strings; `test_esbmc_wrapper.py` passes 6/6. SV-COMP never sets
`--no-unwinding-assertions`, so the line cannot fire there, but the PR carries
`needs-svcomp-run` because the output changed at all.

**Scope, stated rather than implied.** The warning hangs off `report_success()`,
so `--k-induction` and `--incremental-bmc` — which print their verdicts on their
own paths and keep unwinding assertions on to decide when to deepen — do not
reach it. Measured: neither emits it on the reproducer. `--partial-loops`
returns before the counter, so it does not either, which is right: there the
truncation is what the user asked for.

| Artefact | Invocation | Verdict | Mutation it kills |
|---|---|---|---|
| `regression/esbmc/unwind_truncation_warning` | `--unwind 3 --no-unwinding-assertions` | `CORE`, `SUCCESSFUL` **and** the warning | the warning dropped — a control binary from before the patch emits nothing |
| `regression/esbmc/unwind_truncation_warning_absent` | `--unwind 11 --no-unwinding-assertions` | `CORE`, `SUCCESSFUL`, warning absent by negative lookahead | the guard dropped — rebuilt with `if (true)` the twin fails while the first still passes, so the pair is what pins the condition, not either test alone |
| `-L esbmc/`, `-L esbmc-unix` | `-j6` | 1916/1917 and 3 failures | all four pre-existing: `bundled_headers_from_vfs`, and `error`/`error2`/`unsupported_extensions`, which a control binary answers `PARSING ERROR` on this host |
| `scripts/competitions/svcomp/test_esbmc_wrapper.py` | — | 6/6 | — |

What is *not* fixed is R28's unbounded form: with no `--unwind` at all the same
...
**Then the gate does not need a dimension bound at all.** The first patch allowed
`with` chains at exactly two dimensions, on the stated grounds that the flattening
"has no term for" a row. With the encoder fixed that reason is measurably false:

| Probe | master | 2-D-only gate | no dimension gate |
|---|---|---|---|
| 3-D loop bound `a[1][1][1]` | hangs | **hangs** | `SUCCESSFUL` |
| 3-D row read at nondet index | `SUCCESSFUL` | `SUCCESSFUL` | `SUCCESSFUL` |
| 3-D row read through a pointer | `SUCCESSFUL` | `SUCCESSFUL` | `SUCCESSFUL` |

The encoder handled 3-D already; only the gate was refusing it. Dropping the
dimension clause is three lines shorter, fixes the 3-D bound too, and returns the
gate to R42's own framing — bounded by **element count**, not by dimension, with
R42's 256-element cap still doing the cost limiting.
...
| `nested_array_3d_bound{,_fail}` | **hangs** | `SUCCESSFUL`/`FAILED` | `SUCCESSFUL`/`FAILED` | the gate half, at 3-D |
...
| `nested_array_row_ite` | `SUCCESSFUL` | **`FAILED`** | `SUCCESSFUL` | the same, through an `ite` arm |
| `nested_array_row_{via_pointer,ite}_fail` | `FAILED` | `FAILED` | `FAILED` | anti-vacuity twins |

| Artefact | Invocation | Result |
|---|---|---|
| the 10 new tests | default, and again `--z3` | identical verdicts under both solvers |
| `-L esbmc/` | `ctest -j6`, 120 s cap | **2036/2036, 0 failures**, 404 s (master baseline 403 s, same tree minus the patch) |
| unit tests | `ctest -LE regression -j6` | 771/771 |
...
**The trap this entry nearly fell into.** Four of the ten tests report
`0 remaining after simplification`: their claims fold at symex time and the
encoder never sees them. Those four pin the *propagation* half and nothing else —
