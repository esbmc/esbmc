...
| H-A6 | `thread_last_reads/writes` contain *all* accesses of the last transition, including through pointers | **Refuted twice.** R11 → **R18** (one-level resolution losing a nested dereference) was fixed by **#6550**. The completeness this row asks for is now *checked* rather than assumed — a 21-shape census, §15 M9 (H-A6) — and it fails: five shapes holding the pointer in an aggregate are missed, recorded as **R28**. The row stays open, but as a pinned defect rather than an unexamined gap |
...
### M9 (H-A6) — 2026-08-05, the census that found R28
...
`member2t`. Recorded as **R28**, dual-solver confirmed under Bitwuzla and Z3.

**R28 is R18 with the same shape one level out.** #6550 taught the resolution to
...
### M9 (R28 fix) — 2026-08-06, three shapes of five, and why the other two differ
...
`R28FIX ptr=member nil=1 to_global=0`, against
`R28FIX ptr=index nil=0 to_global=1` for the array shape that works. The value
set has no entry for a `member2t` over a struct symbol, so nothing is available
to resolve — this is field sensitivity in the value set, not the resolution gate
R28 named. `mpor_aggregate_ptr_race` stays KNOWNBUG as the marker.

**An asymmetry left open.** `*(u.p)` is *also* `member(symbol, p)` and it
resolves, while `*(s.p)` does not. Union member access presumably lowers
differently before the value set sees it. Not chased here; it is the thread to
pull when the remaining two shapes are taken up, because it suggests the missing
entry is a lowering artefact rather than a deliberate granularity choice.
