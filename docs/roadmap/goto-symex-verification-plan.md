...
| e07 | `struct S { int v[3]; } s[2]`, `&s[0].v[0]`..`&s[1].v[2]` | hangs | hangs | hangs |
...
**The residual.** e07 still hangs: a member access between the two subscripts
ends the index chain, so `&s[0].v[0]` and `&s[1].v[2]` never meet. An array of
structs walked as flat memory is a separate open shape, and — unlike the array
case — the layout offset there depends on padding, so it is not the same
one-line generalisation.
...
| `regression/esbmc` | `ctest -j8`, 120 s cap | 1954 tests, 1 failure: `bundled_headers_from_vfs`, which fails identically on the control (this build links Homebrew LLVM, so `-resource-dir` is not the VFS path) |
