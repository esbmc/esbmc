...
The hazard is real and is about *operands*. But the early return also skipped
the node's own `address_of2t::do_simplify`, which rewrites `&base[c]` to
`&base[0] + c` and never touches `source_value` — the symbol the comment is
protecting. The guard and the fold were never in conflict; the return statement
merged them.
