...
location is a macro expansion gets `_m<spelling offset>` appended, as the
lambda records do (#7530). The spelling offsets of the two `s` differ, so the
ids do too, and a variable declared once still gets one id. Both halves of
