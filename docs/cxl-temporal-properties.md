...
> **Correction.** An earlier revision of this section said the tool that emits
> ESBMC's Büchi format "is neither in this tree nor publicly available". That
> was wrong. It is **https://github.com/esbmc/libltl2ba** — LTL2C by Jeremy
> Morse on LTL2BA 1.1 (Gastin & Oddoux, CAV'01), maintained by Franz Brauße,
> who also wrote `regression/ltl/`. I searched two guessed names
> (`esbmc/ltl2ba`, `ssvlab/ltl2ba`), found neither, and reported a conclusion
> the search did not support. See §7.1 for what the real tool does, and for
> the reason the hand-rolled substitute below nonetheless worked.

`--ltl` needs a Büchi automaton emitted as C. `scripts/ltl_response_ba.py`
...
### 7.1 The real tool, and a version skew

`libltl2ba` builds cleanly (`make`) and has `-O c` for the ESBMC format plus
`-H` for an externs header. `./ltl2ba -O c -f 'G({pressed} -> F {charge > min})'`
reproduces the committed `regression/ltl/basic/test.ba-2.c` almost exactly —
same states, same guards, same acceptance tables — which is independent
confirmation that the hand-transcribed generator below has the automaton right.

But its output **does not work with the ESBMC in this tree**, and the reason is
a signature mismatch:

| | current libltl2ba | `src/esbmc/parseoptions/property_monitors.cpp` |
|---|---|---|
| start | `pthread_t ltl2ba_start_monitor(void)` | inserts a call with **no arguments**, discards the result |
| finish | `ltl2ba_finish_monitor(pthread_t t)` | inserts a call with **no arguments** |
| yielding | `__ESBMC_switch_to/from_monitor()` **enabled** | committed `.ba-N.c` has them commented out |

Feeding the current tool's output to `--ltl` produces no `Final lowest outcome`
line at all — for the reference formula on the reference program, which the
committed automaton reports as `LTL_FAILING`. The committed `.ba-N.c` files
come from an older libltl2ba whose signatures match the instrumentation; the
tool has moved on and ESBMC's side has not.

So the hand-rolled generator worked *because* it was transcribed from the
committed file rather than generated by the current tool. That is luck, not
design. Reconciling the two — updating `property_monitors.cpp` to the current
calling convention, or pinning a compatible libltl2ba revision — is the actual
prerequisite for using arbitrary LTL formulas here, and it is a small,
well-defined change.

Note also what the real tool does with `G !(a && b)`: it emits a **one-state**
automaton with `bad_prefix_states = {false}` and no sink. A violating valuation
makes every transition guard false, so the monitor thread's path becomes
infeasible and is dropped rather than reported. My constructed absence
automaton, with an explicit bad sink, reports `LTL_BAD` where the real tool
reports nothing — which is a difference in encoding philosophy worth
understanding before trusting either for safety properties.
