...
| **M5** | P3 serialisation enum-name exhaustiveness (R7) | `unit/irep2/serialisation.test.cpp` | **Tier B** — sweeps every value of the `constant_string_kindt` / `symbol_renaming_level` / `printf_kindt` / `sideeffect_allockind` enums through the real `type_to_string` overloads asserting a non-empty name (guards the `assert(0 && "Unrecognized…")` tails, `irep2_utils.cpp:449-565`); asserts `get_type_id`/`get_expr_id`/`pretty` are non-empty for representative real nodes; and pins the abort via a POSIX `fork()`/`SIGABRT` death-test on an out-of-range enum (guarded out on Windows). Anti-vacuity: making one enum case return `""` fails the sweep. | 3 cases, 63 assertions PASS | **Done** (this PR). |
