...
worked). The remaining gains are the side-effect-bearing statements (calls,
`++`/`--`) and `code_function_call2t`, which require the IREP2-native
`remove_sideeffects` / `do_function_call` reimplementation the Conclusion prices
as the real cost, plus loops (`code_while2t`/`code_for2t`) and
`code_goto2t`/`code_label2t` (target tracking) — loops are likely higher-value
next given how pervasive they are in real function bodies, `if` included.
