...
worth another native-kind slot. Of the loop kinds, `code_for2t` (real C/C++
`for` syntax) is what remains after the do/while row above; a side-effecting
do/while or `while` condition — the shape `remove_sideeffects` lowers into the
loop preamble, making the continue target an instruction the handler does not
emit itself — is a separate slice from either.
