...
worked). **`code_function_call2t`/side-effect-bearing statements (calls,
`++`/`--`) are now the highest-value next kind**: it's the wall blocking every
desugared Python `for` loop (see the correction in the row above), not just calls in
general — needs the IREP2-native `remove_sideeffects`/`do_function_call`
reimplementation the Conclusion prices as the real cost.
`code_dowhile2t`/`code_for2t` (real for-loop syntax, C/C++-only — Python's is
desugared to `code_while2t` by the preprocessor and already covered) remain
lower priority.
