...
worth another native-kind slot. `code_goto2t`/`code_label2t` (target tracking,
needed for general C `goto`) and `code_dowhile2t`/`code_for2t` (real for-loop
syntax, C/C++-only) remain lower priority than they looked before this row:
the `for`-loop unlock that justified prioritizing loop-adjacent kinds is now
done.
