...
worth another native-kind slot. With `code_for2t` above, every structured loop
form is native; what remains of the structured-CF set is
`code_switch2t`/`code_switch_case2t` — `targets.cases` and the `finish_gotos`
rollback #6178 added are already in place, but `convert_switch`'s case-guard
synthesis and default-label handling are their own slice. A side-effecting loop
condition — the shape `remove_sideeffects` lowers into the loop preamble, so the
continue/back-edge target becomes an instruction the handler does not itself
emit — is a separate slice from any of the loop kinds.
