...
worked). **The real `for`-loop unlock still needs an expression-position call
inside a loop *condition*** to convert — i.e. widening the `while`/`if` guard
checks (currently a blanket `has_sideeffect(cond)` fallback) to recognize the
specific case "guard reduces to exactly one plain, argument-side-effect-free,
result-used call", which needs the full `do_function_call`/`remove_sideeffects`
temp-symbol machinery (the shared-counter byte-identity risk this row's own
narrow slice avoided by requiring the result be unused) — this is the piece the
Conclusion prices as the real cost, now precisely located rather than a general
"side-effect-bearing statements" placeholder. `code_dowhile2t`/`code_for2t`
(real for-loop syntax, C/C++-only — Python's is desugared to `code_while2t` by
the preprocessor and already covered) remain lower priority.
