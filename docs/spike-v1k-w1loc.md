...
convert natively. What remains is not a control-flow shape but the
**side-effecting operand** classes each kind was sliced away from: a
side-effecting loop or switch condition (the shape `remove_sideeffects` lowers
into the preamble, so the continue/back-edge target becomes an instruction the
handler does not itself emit), and a `temporary_object` in an expression
statement (its scope-exit entries die at end-of-full-expression rather than at
block exit, #6177). Beyond those sit the C++-only kinds — `code_cpp_catch2t` /
`code_cpp_throw2t` and the `try`/`catch` target machinery — and the
`code_asm2t` / `code_printf2t` leaves.
