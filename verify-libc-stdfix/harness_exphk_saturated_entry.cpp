...
int main()
{
  /* DEFECT A: EXP_HI[11] is a SACCUM_MAX placeholder -- libc's own comment says
   * "when i = 88 and 89, e_hi will overflow short accum range" -- yet index 88
   * is REACHABLE. The guard rejects x >= 5.5625, but idx 88 needs only
   * x_rounded = 5.5, which every x in [5.4375, 5.5625) rounds to. The
   * placeholder then multiplies exp_mid < 1 and collapses the result.
   *
   * Proved against the oracle, which gives the correctly-rounded exp. */
  short xb = nondet_short();
  __ESBMC_assume(xb >= 696 && xb <= 711);   /* x in [5.4375, 5.5547] */
  ...
  /* Nothing subtle is being claimed: the results differ by far more than any
   * rounding, so even a generous relative bound is blown. */
  int diff = (int)lb - (int)rb;
  if (diff < 0)
    diff = -diff;
  __ESBMC_assert(
    diff * 256 <= (int)rb + 1,
    "exphk on [5.4375, 5.5625): within the documented 2^-8");
  return 0;
}
