/* Validate mkFXPSqrt at unsigned _Fract (u0.16).
...
int main(void)
{
  ...
  unsigned long long xs = (unsigned long long)xb << 16;
  ...
  __ESBMC_assert(lo <= xs, "ur: raw_r^2 <= raw_x * 2^16");
  __ESBMC_assert(hi > xs, "ur: (raw_r+1)^2 > raw_x * 2^16");
  return 0;
}
