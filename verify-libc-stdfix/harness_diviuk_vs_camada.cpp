// diviuk against camada's mkFXPDiv on the same symbolic inputs.
//
// diviuk.cpp calls fixed_point::divifx<unsigned int, unsigned _Accum>(n, d): integer / fixed-point
...
// mean anything, BOTH sides must be representable in unsigned _Accum:
//
...
int main()
{
  ...
  __ESBMC_assume(db >= (1 << 12));

  unsigned int q_libc = fx::divifx<unsigned int, unsigned _Accum>(n, d);

  unsigned _Accum nv = (unsigned _Accum)n;
  unsigned _Accum qv = nv / d;              /* mkFXPDiv */
  unsigned int q_camada = (unsigned int)qv;

  __ESBMC_assert(q_libc == q_camada,
                 "diviuk: divifx agrees with camada's division");
  return 0;
}
