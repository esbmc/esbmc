// diviuk against the contract divifx documents for itself.
//
// diviuk.cpp calls fixed_point::divifx<unsigned int, unsigned _Accum>(n, d) -- integer divided by
...
int main()
{
  ...
  unsigned int q = fx::divifx<unsigned int, unsigned _Accum>(n, d);
  ...
  __ESBMC_assert(aprod <= anum, "diviuk: |q*d| <= |n*2^16| (no overshoot)");
  __ESBMC_assert(aprod + aden > anum, "diviuk: q is the largest such quotient");
  return 0;
}
