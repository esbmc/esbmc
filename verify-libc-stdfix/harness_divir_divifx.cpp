// divir against the contract divifx documents for itself.
//
// divir.cpp calls fixed_point::divifx<int, _Fract>(n, d) -- integer divided by
...
// F = 15 fraction bits, the true quotient is n*2^F / d_raw, so the returned q
...
#define FBITS 15
#define QMAX 2147483647
#define QMIN (-2147483647 - 1)
...
int main()
{
  ...
  /* INT_MIN has no positive counterpart; the truncation identity below needs
   * |n| representable, so exclude it rather than assert something false. */
  __ESBMC_assume(n != (-2147483647 - 1));

  _Fract d;
  __ESBMC_bitcast(&d, &db);

  int q = fx::divifx<int, _Fract>(n, d);
  ...
  __ESBMC_assert(aprod <= anum, "divir: |q*d| <= |n*2^15| (no overshoot)");
  __ESBMC_assert(aprod + aden > anum, "divir: q is the largest such quotient");
  return 0;
}
