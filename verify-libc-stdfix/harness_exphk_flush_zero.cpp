...
int main()
{
  /* DEFECT B: a spurious flush to zero just inside the domain. At index 0 the
   * table entry is EXP_HI[0] = 0.0078125, one ulp, and `lo` can be negative,
   * so exp_hi * (exp_mid * (1 + lo)) underflows to zero. libc's guard only
   * flushes x <= -5.5625, so these inputs are inside the supported domain and
   * their true value rounds to raw 1, not 0:
   *
   *   exp(-5.5390625) = 0.003930210, and half an ulp is 0.003906250
   *
   * so the correctly-rounded result is nonzero. The oracle gives raw 1. */
  short xb = nondet_short();
  __ESBMC_assume(xb >= -709 && xb <= -690);
  ...
  /* The oracle is nonzero throughout this window; libc must not be zero. */
  __ESBMC_assume(rb != 0);
  __ESBMC_assert(lb != 0, "exphk does not flush a representable value to zero");
  return 0;
}
