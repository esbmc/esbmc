// divik against the contract divifx documents for itself.
//
// divik.cpp calls fixed_point::divifx<int, _Accum>(n, d) -- integer divided by
...
int main()
{
  ...
  int q = fx::divifx<int, _Accum>(n, d);
  ...
  __ESBMC_assert(aprod <= anum, "divik: |q*d| <= |n*2^15| (no overshoot)");
  __ESBMC_assert(aprod + aden > anum, "divik: q is the largest such quotient");
  return 0;
}
