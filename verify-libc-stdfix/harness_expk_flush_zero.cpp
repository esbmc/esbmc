...
int main()
{
  /* Concrete rather than symbolic: correctly-rounded exp at s16.15 needs a
   * 37-bit intermediate (camada's measured hardest-to-round bound), and a
   * symbolic sweep over that window does not finish in reasonable time. These
   * four inputs are natively measured -- exp is above half an ulp at each, so
   * the correctly-rounded result is raw 1 -- and the oracle agrees. */
  int xb = nondet_int();
  __ESBMC_assume(
    xb == -363391 || xb == -363000 || xb == -362000 || xb == -360000);
