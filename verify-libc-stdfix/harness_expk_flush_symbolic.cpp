...
int main()
{
  /* Symbolic over the whole flush window (3392 inputs), not pinned values.
   *
   * SOLVER NOTE: this needs --z3. Correct rounding at s16.15 requires a 37-bit
   * intermediate (camada's measured hardest-to-round bound), and bitwuzla did
   * not finish this query in 90 minutes; z3 discharges it in ~41s. The
   * narrow-window variant (32 inputs) does finish under bitwuzla in 2.7s. */
  int xb = nondet_int();
  __ESBMC_assume(xb >= -363391 && xb <= -360000);
