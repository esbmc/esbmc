...
c_typecastt::follow_with_qualifiers(const type2tc &src_type)
{
  ...
  // We ditch qualifiers during symex, which is the only place that irep2 exists
  // right now. If we ever move frontends over to new irep, then this will
  // become an issue, hence the following warning.
#warning jmorse: irep2 qualifiers dont exist
  ...
  c_typet c_type)
{
  ...
  if(new_type!=expr_type)
  {
    if (is_pointer_type(new_type) && is_array_type(expr_type))
    {
      ...
      index2tc index_expr(arr_type->subtype, expr, zero_uint);
  ...
  const type2tc &dest_type)
{
  if (is_pointer_type(dest_type))
  {
    ...
    if (is_constant_int2t(expr) &&
        to_constant_int2t(expr).constant_value.to_long() == 0 && (
...
void c_typecastt::do_typecast(expr2tc &dest, const type2tc &type)
{
  ...
  if (is_array_type(dest_type))
  {
    ...
    index2tc index(arr_type->subtype, dest, zero_uint);
