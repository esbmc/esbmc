...
void reachability_treet::check_mutex(const exprt &code, const execution_statet &ex_state)
{
  ...
  {
	if (object.op0().type().get_string(exprt::a_identifier).find("pthread_mutex") != std::string::npos)
	{
	  if (is_first_assign)
	  {
		if (object.op0().operands().size()==0)
		  return;
	    identifier = object.op0().op0().get_string(exprt::a_identifier);
	    is_first_assign=false;
	  }

	  val = integer2string(binary2integer(value.get_string("value"), true),10);

	  if (identifier.find(object.op0().op0().get_string(exprt::a_identifier)) != std::string::npos)
	    _is_same_mutex=true;
