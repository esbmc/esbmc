...
//  std::cout << "generate_states_base expr.pretty(): " << expr.pretty() << std::endl;
  ...
  for(unsigned int i = 0; i < ex_state._threads_state.size(); i++)
  {
    ...
    // If the thread we're switching to isn't already active, then a context
    // switch has occured. Increment the counter for this.
    if (i != new_state->get_active_state_number()) {
