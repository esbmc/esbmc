...
REGMAP_TABLE m_RegMapTable = {m_RegMap};

REGMAP_TABLE m_PORT_RegMapTable = {m_PORT_RegMap};
...
UINT64 RegWrite(REGMAP_TABLE *RegMapTable, UINT32 EntryIndex, UINT64 Data64)
{
  if(RegMapTable == NULL)
  {
    return 0;
...
UINT64 RegRead(REGMAP_TABLE *RegMapTable, UINT32 EntryIndex)
{
  if(RegMapTable == NULL)
  {
    return ~0;
...
int main()
{
  ...
  __ESBMC_assert(
    pInst->PortIsDisabled[aux_index] ||
      (read_data1 == 0 && read_data2 == 0 && read_data3 == 0),
