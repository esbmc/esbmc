/* ADASModel_func_fail: Test function contracts with struct types - FAIL case
 * Tests __ESBMC_return_value with WRONG contracts that should fail verification
 * Demonstrates error detection for incorrect ensures clauses
 * This design is from https://github.com/lf-lang/lf-verifier-benchmarks. But I implemented the code in C.
 */
#include <assert.h>
#include <stdbool.h>
#include <stddef.h>
...
// Test functions with WRONG contracts (will fail)
int get_frame_value(c_frame_t frame) {
    __ESBMC_requires(frame >= 0);
    // WRONG: says return value equals frame, but actually returns frame * 2
    __ESBMC_ensures(__ESBMC_return_value == frame); // This will fail!
    return (int)frame * 2;  // Actually returns frame * 2
}
...
double calculate_delay(unsigned int time1, unsigned int time2) {
    ...
    // WRONG: says return value equals (time2 - time1), but actually returns (time2 - time1) * 2.0
    __ESBMC_ensures(__ESBMC_return_value == (double)(time2 - time1)); // This will fail!
    return (double)(time2 - time1) * 2.0;  // Actually returns (time2 - time1) * 2.0
}
...
int main() {
    ...
    // Test function with int return type and WRONG contract (will fail)
    int frame_val = get_frame_value(lidar_out);
    assert(frame_val == 4);  // Expect frame * 2 = 2 * 2 = 4
    ...
    // Test function with double return type and WRONG contract (will fail)
    double delay = calculate_delay(0, current_time);
    assert(delay == 100.0);  // Expect (50 - 0) * 2 = 100.0
