...
def test_string_ternary_with_boolean_conditions() -> None:
    ...
    subscription_message: str = "premium" if (is_logged_in and is_premium) else ("basic" if is_logged_in else "guest")
...
def test_nested_string_ternary() -> None:
    level: int = 75
    performance: str = "excellent" if level >= 90 else ("good" if level >= 70 else ("average" if level >= 50 else "poor"))
    
    # Different level values
    high_level: int = 95
    high_performance: str = "excellent" if high_level >= 90 else ("good" if high_level >= 70 else ("average" if high_level >= 50 else "poor"))
    assert high_performance == "excellent"
    
    low_level: int = 30
    low_performance: str = "excellent" if low_level >= 90 else ("good" if low_level >= 70 else ("average" if low_level >= 50 else "poor"))
...
def test_string_truthiness_simulation() -> None:
    ...
    first_name: str = ""
    last_name: str = "Smith"
    display: str = "full_name" if (first_name != "" and last_name != "") else ("last_only" if last_name != "" else "no_name")
