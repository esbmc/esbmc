...
class CTest1(ParseTest):
    ...
    def setUp(self):
        self.test_case: CTestCase = CTestCase("./esbmc/00_bbuf_02", "00_bbuf_02")
        self.test_parsed: CTestCase = TestParser.from_file("./esbmc/00_bbuf_02", "00_bbuf_02")

    def _read_file_checks(self, test_obj):
        self.assertEqual(test_obj.test_mode, "CORE")
        self.assertEqual(test_obj.test_file, "main.c")
        self.assertEqual(test_obj.test_args, "--unwind 1 --context-bound 2 --schedule --depth 300")
...
class CTest3(ParseTest):
    ...
    def setUp(self):
        self.test_case: CTestCase = CTestCase("./esbmc/00_account_02", "00_account_02")
        self.test_parsed: CTestCase = TestParser.from_file("./esbmc/00_account_02", "00_account_02")
...
class CTest4(ParseTest):
    ...
    def setUp(self):
        self.test_case: CTestCase = CTestCase("./esbmc/29_exStbHwAcc", "29_exStbHwAcc")
        self.test_parsed: CTestCase = TestParser.from_file("./esbmc/29_exStbHwAcc", "29_exStbHwAcc")

    def _read_file_checks(self, test_obj: BaseTest):
        self.assertEqual(self.test_case.test_mode, "CORE")
        self.assertEqual(self.test_case.test_file, "main.c")
        self.assertEqual(self.test_case.test_args, "--overflow-check  --unwind 3 --32")
        self.assertEqual(self.test_case.test_regex, ["^VERIFICATION FAILED$"])

    def _argument_list_checks(self, test_obj: BaseTest):
        argument_list = self.test_case.generate_run_argument_list("__test__")
        expected = ['__test__', '--overflow-check', '--unwind', '3', '--32', 'main.c']
        self.assertEqual(argument_list, expected, str(argument_list))
...
class XMLTest1(ParseTest):
    ...
    def setUp(self):
        self.test_case: XMLTestCase = XMLTestCase("./esbmc-cpp/cpp/ch1_0", "ch1_0")
        self.test_parsed: XMLTestCase = TestParser.from_file("./esbmc-cpp/cpp/ch1_0", "ch1_0")

    def _read_file_checks(self, test_obj: BaseTest):
        self.assertEqual(self.test_case.test_mode, "CORE")
        self.assertEqual(self.test_case.test_file, "main.cpp")
        self.assertEqual(self.test_case.test_args, "--unwind 10 --no-unwinding-assertions -I ~/libraries/ --memlimit 22000000 --timeout 60m")
        self.assertEqual(self.test_case.test_regex, ["^VERIFICATION FAILED$"])
