// CXL DVSEC range decode over PCI config space with the range-count rejection
// dropped.
//
// The synthetic counterpart to harness_dvsec_rr_decode_fail: config space is
// hardware state, so hdm_count is whatever the device reports. Without the
// rejection it indexes a 2-entry array.
                            ...
                            struct cxl_endpoint_dvsec_info *info)
{
  ...
  /*
   * BUG: no rejection of hdm_count against the size of dvsec_range[].
   * Reading the count back from the device is not validating it -- the
   * register is exactly the thing that cannot be trusted.
   */
  info->ranges = 0;
