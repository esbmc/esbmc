...
void compare_with_round_to_even() {
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        // Verify they differ for ties
        if (fmod(fabs(val), 1.0) == 0.5) {
            // This is a tie - ROUND_TO_AWAY should differ from round-to-even
            // for some cases (e.g., 1.5, 3.5 should differ)
            if (fabs(val) == 1.5 || fabs(val) == 3.5) {
                assert(away_result != std_result);
            }
        }
    }
}
