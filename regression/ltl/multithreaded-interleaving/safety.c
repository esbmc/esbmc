#if 0
generated by libltl2ba with command: /private/tmp/claude-502/-Users-user-esbmc-build/2dd3b8c5-df77-4f49-8a49-b881795618cb/scratchpad/libltl2ba/ltl2ba -O c -H "tau.h" -f F {s != 0}
...
Accepting pessimistic cycles: {1}
Accepting pessimistic states: {1}
#endif
#include <pthread.h>
#include <stdbool.h>
#include <stdint.h>
...
#include "tau.h"
char __ESBMC_property__ltl2ba_cexpr_0[] = "s != 0";
int _ltl2ba_cexpr_0_status(void) { return s != 0; }
...
ltl2ba_fsm(bool state_stats, unsigned int num_iters)
{
	...
	for (iters = 0; iters < num_iters; iters++) {
		...
		case _ltl2ba_state_0:
			state_is_viable = (((1)) || ((_ltl2ba_cexpr_0_status())) || ((false)));
			if (choice == 0) {
				__ESBMC_assume(((1)));
				_ltl2ba_statevar = _ltl2ba_state_0;
			} else if (choice == 1) {
				__ESBMC_assume(((_ltl2ba_cexpr_0_status())));
...
{
	unsigned int idx = 0;
	idx |= (_ltl2ba_cexpr_0_status()) ? 1 << 0 : 0;
	return idx;
}
