...
/* #4423's reproducer, re-run with --sleep-sets to pin that the reduction stays
 * off under --data-races-check-only: that flag makes get_expr_globals record
 * nothing, so every footprint is empty, no sleeping thread can ever wake, and
 * this race was silently lost again (issue #6831).
 *
