...
#define BOUNDED_BUF_INITIALIZER                                                \
  {                                                                            \
    BOUNDED_BUF_VALID, PTHREAD_MUTEX_INITIALIZER, PTHREAD_COND_INITIALIZER,    \
      PTHREAD_COND_INITIALIZER, 0, 0, 0, 0, null                               \
  }
...
int bounded_buf_destroy(bounded_buf_t *bbuf)
{
  ...
  {
    for (i = bbuf->head; i < bbuf->rear; i++)
      free(bbuf->buf[i]);
  }
  else
  {
    for (i = bbuf->head; i < bbuf->max_size; i++)
      free(bbuf->buf[i]);
    for (i = 0; i < bbuf->rear; i++)
      free(bbuf->buf[i]);
  }
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < PRODUCER_SUM; i++)
  {
    ...
    pthread_create(
      &producers[i].pid, NULL, producer_routine, (void *)&producers[i]);
  }
  ...
  for (i = 0; i < CONSUMER_SUM; i++)
  {
    ...
    pthread_create(
      &consumers[i].pid, NULL, consumer_routine, (void *)&consumers[i]);
  }
