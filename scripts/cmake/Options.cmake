...
elseif(NOT APPLE AND CMAKE_SYSTEM_PROCESSOR MATCHES "^(aarch64|arm64)$")
  ...
  # No LLVM is downloaded here. The only LLVM 22 aarch64 release is the full
  # install tree, 11GB extracted, which does not fit a public ARM runner, so
  # build.sh installs the distro llvm packages and points LLVM_DIR/Clang_DIR at
  # them; SetupLocalLLVM downloads only when LLVM_DIR is empty. Z3 still comes
  # from a prebuilt: its arm64 archive is small and ships the libz3.a that a
  # static link needs.
...
# Every URL above other than Z3 is published for x86_64 only, and an unset one
# would otherwise reach download_zip_and_extract as an empty string. Say what is
...
if(ESBMC_AARCH64_LINUX)
  ...
  if(DOWNLOAD_DEPENDENCIES AND "${LLVM_DIR}" STREQUAL "")
    message(FATAL_ERROR "No prebuilt LLVM is available for aarch64. Install llvm/clang >= ${MIN_SUPPORTED_LLVM_VERSION_MAJOR} and pass -DLLVM_DIR and -DClang_DIR, as scripts/build.sh does.")
  endif()
endif()
