...
def _init_worker(index):
    """Pool initializer: install the shared line index once per worker process.

    ProcessPoolExecutor.map would otherwise pickle and transfer the full
    index (hundreds of thousands of entries on the real repo) with every
    job tuple instead of once per worker.
    """
