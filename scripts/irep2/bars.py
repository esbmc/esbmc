...
*2tc constructor, or a name ending in `2tc`/`2t`. It does not resolve types, so a
write passing an IREP2 variable under another name still counts as debt -- the refined
figure is an upper bound, and a tighter one than the raw.
"""
import re
import subprocess
import sys
