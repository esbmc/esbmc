#!/bin/bash
# Full-corpus verdict census of every C++ regression row under
# --clang-cpp-irep2-adjust-only, against the default path. One TSV line per row:
...
run_row() {
  ...
  on=$(  (cd "$d" && timeout 45 "$E" "$src" "${flag_words[@]}" --clang-cpp-irep2-adjust-only 2>&1) )
...
export -f run_row
find regression/esbmc-cpp regression/esbmc-cpp11 regression/esbmc-cpp14 \
     regression/esbmc-cpp17 regression/esbmc-cpp20 regression/esbmc-cpp23 \
     -name test.desc | sort \
