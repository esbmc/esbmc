...
def report(rows):
    ...
    if counted and not any(
        ...
        print("\nNOTE: no run reported a cache line, so --vcc-cache never "
              "activated. It needs --multi-property, which the competition "
              "command does not use; these timings say nothing about the "
              "cache.")
