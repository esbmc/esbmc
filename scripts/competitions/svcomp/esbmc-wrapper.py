...
def get_command_line(strat, prop, arch, benchmark, concurrency, dargs):
  ...
  if prop == Property.overflow:
    ...
    if concurrency:
      ...
      command_line += "--no-pointer-check --no-bounds-check --interval-analysis --error-label ERROR --goto-unwind --unlimited-goto-unwind --interval-analysis-wrapped --interval-analysis-arithmetic --interval-analysis-bitwise "
  else:
