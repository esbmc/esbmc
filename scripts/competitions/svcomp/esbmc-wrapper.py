...
def get_command_line(strat, prop, arch, benchmark, concurrency, dargs, esbmc_ci):
  ...
  if prop == Property.overflow:
    ...
    command_line += "--memory-leak-check --no-reachable-memory-leak --no-assertions --bitwuzla "
    ...
    command_line += "--no-pointer-check --no-bounds-check --memory-leak-check --no-assertions --bitwuzla "
    strat = "incr"
  elif prop == Property.reach:
    command_line += "--enable-unreachability-intrinsic  --bitwuzla "
    if concurrency:
    ...
    command_line += "--no-pointer-check --no-bounds-check --data-races-check --no-assertions --bitwuzla "
  else:
