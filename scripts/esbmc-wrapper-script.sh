...
global_cmd_line="--64 -DLDV_ERROR=ERROR -Dassert=notassert -D_Bool=int --no-bounds-check --no-pointer-check --error-label ERROR --no-div-by-zero-check --no-assertions --quiet --context-switch 4 --force-malloc-success"

# The simple memory model command line is the global, without all the
# safety checks.
memory_cmd_line="--64 -DLDV_ERROR=ERROR -Dassert=notassert -D_Bool=int --quiet --context-switch 3 --force-malloc-success --memory-leak-check"
