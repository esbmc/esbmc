...
def fmt_time(t, expected, verdict):
    if t in ('N/A', '', None):
        return 'N/A'
    try:
        f = float(t)
...
def main(csv_path):
    ...
    lines.append(r'\caption{ESBMC-PLC+ vs NuXmv BDD and IC3 — performance comparison on IEC~61131-3 LD benchmarks. ' +
                 r'ESBMC uses \textit{k}-induction with Z3; NuXmv BDD uses CUDD-based BDD reachability; NuXmv IC3 uses ' +
                 r'the IC3/PDR algorithm. Timeout: 120\,s. INT columns show the number of integer-valued ' +
                 r'timer/counter state variables.}')
    lines.append(r'\label{tab:nuxmv_comparison}')
    lines.append(r'\resizebox{\textwidth}{!}{%')
    lines.append(r'\begin{tabular}{lrrr|rr|rr|rr}')
    lines.append(r'\toprule')
    lines.append(r'\multirow{2}{*}{\textbf{Benchmark}} & \multirow{2}{*}{\textbf{INT}} & \multirow{2}{*}{\textbf{Rungs}} & \multirow{2}{*}{\textbf{Props}} &')
    ...
    lines.append(r' & & & & \textbf{Result} & \textbf{Time} & \textbf{Result} & \textbf{Time} & \textbf{Result} & \textbf{Time} \\')
    ...
    for row in rows:
        ...
        e_time = fmt_time(et,  exp, ev)
        b_time = fmt_time(bt,  exp, bv)
        i_time = fmt_time(it_, exp, iv)
