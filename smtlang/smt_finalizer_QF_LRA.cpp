...
Function: smt_finalizer_QF_LRAt::finalize
...
bool smt_finalizer_QF_LRAt::finalize(const exprt& benchmark)
{
  ...
  smt_typecheck_expr_QF_LRAt checker(err, benchmark, empty);
...
void smt_finalizer_QF_LRAt::check_double_sorts(
  ...
  message_streamt &err) 
{
  ...
  {
    if(smt_theory_Reals().check_sorts(ii2string(*sit)))
    {
      err.str <<
...
Function: smt_finalizer_QF_LRAt::check_double_functions
...
void smt_finalizer_QF_LRAt::check_double_functions(
  const exprt &benchmark, 
  const symbolst &theories,
  message_streamt &message) 
{
  ...
  { 
    ...
    typet t = 
      smt_theory_Reals().check_functions(ii2string(*fit), params);
...
Function: smt_finalizer_QF_LRAt::check_double_predicates
...
void smt_finalizer_QF_LRAt::check_double_predicates(
  const exprt& benchmark, 
  const symbolst& theories,
  message_streamt &err) 
{
  ...
  { // for all functions
    ...
    if(smt_theory_Reals().check_predicates(ii2string(*pit), params))
    {
      err.str <<
...
Function: smt_typecheck_expr_QF_LRAt::search_fun
...
typet smt_typecheck_expr_QF_LRAt::search_fun(
  const exprt &e,
  const typet &params) 
{  
  if (smt_theory_Reals().check_predicates(ii2string(e), params))
    return typet("bool");
  else   
  {
    typet t = 
      smt_theory_Reals().check_functions(ii2string(e), params);
    if (t.id()!="") return t; 
  }
...
Function: smt_typecheck_expr_QF_LRAt::search_sort
...
typet smt_typecheck_expr_QF_LRAt::search_sort(const typet& sort)
{  
  if (smt_theory_Reals().check_sorts(ii2string(sort)))
